...
function heightAtYato(x, z) { // 実標高をバイリニア補間。zは反転サンプル(データ側を北=-zにしたため)。±SG.half外は縁の値で頭打ち
  ...
  let h = Math.max(2, e + (e - 18) * 0.8) // 鉛直強調1.8倍(基準18m)：DEM5A(5mメッシュ)が均す急な切土/坂を、丘谷の“形”はそのままに急勾配へ。位置は不変
  ...
  // 三ツ池公園のプール＝丘の斜面(西33m〜東19m)を水平な段地(H0=24m)に均す。縁はsmoothstepで地形へなだらかに（プールの底地・ユーザー要望2026-06-25）
  if (x > 3879 && x < 3927 && z > -729 && z < -679) { const dxr = Math.abs(x - 3903), dzr = Math.abs(z + 704)
    const k = smoothstep01((21 - dxr) / 7) * smoothstep01((18 - dzr) / 8); if (k > 0) h = h * (1 - k) + 24 * k // 平地はデッキ(23x15)より広く＝デッキの縁が宙に浮かない
    // プールの槽（開口部）だけ掘り下げる＝水の下に段地の地面が透けない（内側を22.2mへ）
    const dxp = Math.abs(x - 3903), dzp = Math.abs(z + 705); const kp = smoothstep01((7 - dxp) / 1.3) * smoothstep01((4 - dzp) / 1.3); if (kp > 0) h = h * (1 - kp) + 22.2 * kp }
  return h
}
...
function buildShishigaya() {
  ...
  const SLIDE_TOP = [3786, -889], SLIDE_END = [3817, -845], POOL = [3903, -704] // プール中心（段地・makePoolと共有）
  ...
  { const cx = POOL[0], cz = POOL[1], H0 = 24, deckY = H0 + 0.12, g = new THREE.Group()
  ...
  { const roadOcc = new Uint8Array(GC * GC) // 道の通るセルは草を生やさない（路面に草が刺さらない。セル6mなので路肩1mほどから生える）
    ...
    const bareZones = [[3124, -186, 31, 51], [3062, -150, 5, 5], [3050, -161, 16, 4.5], [3069, -157, 11, 4.5], [3055, -104, 14, 11], [3903, -704, 14, 9]] // 草を生やさない裸地＝[校庭][広場の池][裏門→校舎沿いの一本道(2区間)][学校プール][三ツ池プールのデッキ]。広場の残りは緑(夏草)で覆う＝ユーザー要望。マリノスG(雑草原っぱ)は除外しない
