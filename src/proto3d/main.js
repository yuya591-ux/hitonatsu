...
const townNightLights = [] // 夜に灯る街のあかり（窓・街灯・自販機）。nfで点灯＝夜のエモさ
{
  ...
  function makeGround(cx, cz) {
    ...
    const dirt = new THREE.Mesh(new THREE.PlaneGeometry(W, D), new THREE.MeshToonMaterial({ color: 0xc6a87a, gradientMap: GRAD, map: watercolorTex })); dirt.rotation.x = -Math.PI / 2; dirt.position.set(cx, fy + 0.04, cz); dirt.receiveShadow = true; scene.add(dirt)
    const lineMat = new THREE.MeshToonMaterial({ color: 0xeee8da, gradientMap: GRAD })
