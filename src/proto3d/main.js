...
const SLOPE = { x: TOWN.x - 78, z0: TOWN.z - 96, z1: TOWN.z + 4, h: 30, w: 34 } // 尾根の道のx・幅。大きく
...
const townNightLights = [] // 夜に灯る街のあかり（窓・街灯・自販機）。nfで点灯＝夜のエモさ
{
  ...
  makeRoadRibbon(T.x - 78, T.z + 46, T.x - 78, T.z - 92, 9) // 長く大きい坂道（尾根の道）：北(下/しんみせ) → 南(上/マンション・頂上)。マンションは坂の約7割地点の西脇
  // 上り(南)＝頂上→北寺尾の町方面／下り(北)＝ビスコ(踊り場)→しんみせ(一番下)→本通り
  makeRoadRibbon(T.x - 78, T.z + 42, T.x - 4, T.z + 8, 6) // 坂下(北)を町の本通りへ接続
  makeRoadRibbon(T.x - 80, T.z - 50, T.x - 87, T.z - 50, 5, false) // マンションへ左折で入る短い下り坂(私道)：尾根の道→西へ一段下って入口へ
  ...
  // ── マンション背面(西)から出る細い道（一方通行幅）→ 森を左(南)から迂回 → 小学校へ ──
  makeRoadRibbon(T.x - 96, T.z - 52, T.x - 118, T.z - 66, 3.6, false) // 背面の出口→南西へ（森の南をかすめる）
  makeRoadRibbon(T.x - 118, T.z - 66, T.x - 142, T.z - 46, 3.6, false) // 西へ回り込み小学校方向へ登り返す
  makeRoadRibbon(T.x - 142, T.z - 46, T.x - 148, T.z - 30, 3.6, false) // 小学校の前へ
  ...
  makeGround(T.x - 142, T.z - 72) // マリノスのグラウンド＝森を迂回して小学校へ向かう道中の中ほど・左手(南)
  ...
  function makeGameShop(cx, cz, rot) {
    ...
    mergedOutline(g, 0.05); addContactShadow(g, 8.6); addCollider(cx, cz, 5.6); scene.add(g)
