...
const bonOdori = new THREE.Group(); bonOdori.visible = false; scene.add(bonOdori) // 盆踊り会場（櫓＋提灯）＝小学校の校庭。開催日だけ姿を見せる
{
  ...
  // ※ユーザー要望：土の散歩道→「車1台分くらいの細い舗装(コンクリート)道路」に変更。幅3.4・センターラインなし・concrete=true・地形に乗せるlift。
  const IKEW = 3.4 // 二つ池への細い舗装路の幅（一車線）
  makeRoadRibbon(T.x - 78, T.z + 42, T.x - 97, T.z + 42, IKEW, false, true, 0.05)  // 交差点：尾根道→右折して西へ（しんみせの南を通る＝しんみせは右手）
  makeRoadRibbon(T.x - 97, T.z + 42, T.x - 116, T.z + 37, IKEW, false, true, 0.05) // 西へ（しんみせを右手に見送る）
  makeRoadRibbon(T.x - 116, T.z + 37, T.x - 134, T.z + 44, IKEW, false, true, 0.05)// 一うねり：北へ膨らむ
  makeRoadRibbon(T.x - 134, T.z + 44, T.x - 153, T.z + 37, IKEW, false, true, 0.05)// 一うねり：南へ戻る（S字ひとつ）
  ...
  // ── 【裏山の谷を下る3本目の道／ユーザー要望2026-06-18：旧・直線の谷道(924,228まで)を消し、指定5点をたどる“西へカーブして下る道”に作り直し】──
  // 二股(西の丘/見晴らし)の“間”(922,88)から、森(西の丘ぞい)の中を西へゆるくカーブしながら下り、北の平らな新しい土地へ抜ける（地形に沿って下る：14.7→2.4→0m）
  makeRoadRibbon(T.x - 78, T.z + 88, T.x - 81, T.z + 99, 4, false, true)    // (922,88)→(919,99) 分岐から下り始め＝地点1
  makeRoadRibbon(T.x - 81, T.z + 99, T.x - 88, T.z + 101, 4, false, true)   // (919,99)→(912,101) 地点2
  makeRoadRibbon(T.x - 88, T.z + 101, T.x - 96, T.z + 106, 4, false, true)  // (912,101)→(904,106) 地点3
  makeRoadRibbon(T.x - 96, T.z + 106, T.x - 102, T.z + 114, 4, false, true) // (904,106)→(898,114) 地点4
  makeRoadRibbon(T.x - 102, T.z + 114, T.x - 112, T.z + 136, 4, false, true)// (898,114)→(888,136) 地点5（指定の最後）
  makeRoadRibbon(T.x - 112, T.z + 136, T.x - 120, T.z + 155, 4, false, true)// (888,136)→(880,155) 同じ向きでさらに下る
  makeRoadRibbon(T.x - 120, T.z + 155, T.x - 128, T.z + 178, 4, false, true)// (880,155)→(872,178) 山を抜ける
  makeRoadRibbon(T.x - 128, T.z + 178, T.x - 132, T.z + 205, 4, false, true)// (872,178)→(868,205) 平らな新しい土地へ出る
...
;(function setupPlaceMap() {
  ...
  const LM = [
    ...
    { x: 884, z: 150, t: '谷の道', k: 'road', dy: -11 },                        // 西へカーブして下る新しい谷の道(922,88→868,205)の中ほど
  ]
