...
const townNightLights = [] // 夜に灯る街のあかり（窓・街灯・自販機）。nfで点灯＝夜のエモさ
{
  ...
  function makeMansion(cx, cz) {
    ...
    for (let u = 0; u < units; u++) { const op = new THREE.Mesh(new THREE.PlaneGeometry(1.7, 1.9), toon(0x26262a)); op.position.set(-W / 2 + 1.3 + u * 2.6, baseH * 0.55, -D / 2 - 0.03); op.rotation.y = Math.PI; g.add(op) } // 駐車場の開口＝道路側(-z)
    // 居室7階（グレー）
