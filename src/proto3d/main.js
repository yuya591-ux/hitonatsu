...
const YATO = {
  x: 3000, z: 0,
  // 谷戸の中心線＝実在の陰影起伏図をなぞった近似。各点 [x, z, 谷底標高]
  v_futatsu: [[3000, 150, 12], [2995, 320, 6], [2990, 500, 3.5], [2985, 660, 3]], // 二ツ池の谷（北：中心→二ツ池→鶴見川低地）
  v_west: [[2950, 90, 15], [2820, 150, 8], [2680, 210, 5]],     // 西の谷（師岡・上の宮側へ）
  v_east: [[3060, 70, 15], [3250, 170, 8], [3430, 250, 5]],     // 東の谷（末吉・鶴見川側へ）
  v_south: [[2990, -160, 16], [2965, -380, 10], [2950, -600, 6]], // 南の谷（馬場・西寺尾側へ）
  v_sw: [[2900, -40, 17], [2760, -120, 9], [2640, -200, 6]]      // 南西の枝谷
}
...
function heightAtYato(x, z) {
  const Y = YATO, lx = x - 3000, lz = z // +x=東/+z=北/原点=中心33.8m
  let h = 33 // 下末吉台地の基準面
  h -= smoothstep01((lz - 250) / 480) * 29   // 北の鶴見川低地へ（z+250→+730 で 33→4）
  h -= smoothstep01((-280 - lz) / 460) * 23  // 南の鶴見低地へ（z-280→-740 で 33→10）
  h -= smoothstep01((-520 - lx) / 360) * 12  // 西の端（師岡台地の先）で少し下る
  h -= smoothstep01((lx - 560) / 360) * 8    // 東の端（谷の先）で少し下る ※三ツ池の丘で上書き
  const mdx = lx - 650, mdz = lz - 640       // 三ツ池の丘（北東・peak~55m）
  const mit = 55 * Math.exp(-(mdx * mdx + mdz * mdz) / (2 * 290 * 290))
  if (mit > h) h = mit
  // 台地を樹枝状に刻む小起伏（陰影起伏図の“刻まれた”質感）＝台地部だけ
  const tex = Math.abs(Math.sin(lx * 0.021) * Math.cos(lz * 0.019)) * 0.6 + Math.abs(Math.sin((lx + lz) * 0.013)) * 0.4
  h -= (1 - tex) * 3.5 * smoothstep01((h - 10) / 8)
  // 主要な谷戸（実在の陰影起伏図をなぞる）
  h = carveValley(h, x, z, Y.v_futatsu, 16, 34) // 二ツ池の谷（北）
  h = carveValley(h, x, z, Y.v_west, 13, 28)    // 西の谷
  h = carveValley(h, x, z, Y.v_east, 13, 28)    // 東の谷
  h = carveValley(h, x, z, Y.v_south, 14, 30)   // 南の谷
  h = carveValley(h, x, z, Y.v_sw, 11, 24)      // 南西の枝谷
  const pdx = x - 2990, pdz = z - 500, pdd = Math.sqrt(pdx * pdx + pdz * pdz) // 二ツ池の窪地
  const pondK = smoothstep01((22 - pdd) / 12)
  if (pondK > 0 && h > 3) h = h * (1 - pondK) + 3 * pondK
...
{
  const Y = YATO
  const YGX = Y.x, YGZ = Y.z // 地面メッシュの中心＝原点（実スケール 1.4km×1.5km）
  const ygeo = new THREE.PlaneGeometry(1400, 1500, 280, 300); ygeo.rotateX(-Math.PI / 2) // ~5m格子。核心は実寸1:1
  ...
  for (let i = 0; i < ypos.count; i++) {
    ...
    const c = cLow.clone().lerp(cGrass, THREE.MathUtils.smoothstep(y, 2, 8))
    c.lerp(cDark, THREE.MathUtils.smoothstep(y, 14, 34))
  ...
  // 谷戸田（各谷の底＝実位置）
  for (const [px, pz, w, d] of [[2996, 200, 18, 28], [2992, 300, 18, 28], [2990, 410, 16, 24], // 二ツ池の谷（北）
    [2850, 150, 14, 22], [2740, 195, 12, 18],                                                    // 西の谷
    [3180, 150, 14, 22], [3320, 215, 12, 18],                                                    // 東の谷
    [2975, -280, 16, 26], [2962, -440, 14, 22], [2760, -135, 12, 18]]) makeRicePaddy(px, pz, w, d) // 南・南西の谷
  makeStream([[3000, 120], [2997, 270], [2993, 400], [2989, 520], [2986, 650]], 2.4) // せせらぎ＝二ツ池の谷を北へ
  let tp = 0, tt = 0 // 雑木林＝市民の森(北の丘)・三ツ池丘(北東)・台地の縁。谷底/低地/中心の宅地ゾーンは避ける
  while (tp < 230 && tt < 5000) {
    tt++
    const x = 2360 + Math.random() * 1280, z = -680 + Math.random() * 1380, y = heightAt(x, z)
    if (y < 18) continue
    if (Math.abs(x - 3000) < 230 && Math.abs(z) < 200 && Math.random() < 0.7) continue // 中心の宅地は少し空ける
    makeTree(x, z, 0.8 + Math.random() * 0.8); tp++
  }
}
...
}
{
  ...
  // ───────── 新エリア『獅子ヶ谷（実地形）』ランドマークを実位置で再配置（中心=サンライズ北寺尾／+x=東/+z=北） ─────────
  makeDanchi(3000, 14, 0, 7)        // サンライズ北寺尾（中心の7階建マンション・1995竣工）＝台地の上
  makePondPark(2990, 500)           // 二ツ池（北の谷）
  makeYokomizo(2860, 350)           // 横溝屋敷（御園＝二ツ池の谷の口・北西）
  makePondPark(3500, 560)           // 三ツ池公園（北東の丘＝台地上の池・桜の名所）
  makeRoadRibbon(2560, 150, 3000, 70, 7, true, true)    // 主要道：西(師岡)→中心
  makeRoadRibbon(3000, 70, 3380, 150, 7, true, true)    // 中心→東(末吉/三ツ池)
  makeRoadRibbon(3000, 60, 2992, 360, 5, false, true)   // 中心→北(二ツ池/横溝)へ下る道
  makeRoadRibbon(3000, -30, 2985, -360, 5, false, true) // 中心→南(馬場)へ下る道
  makeKurumaJizo(2952, 240)         // 車地蔵（谷へ降りる道のかたわら）
  makeSignpost(2980, 300, 0, 'ふたつ池 ／ 横溝屋敷 →') // 谷戸の道しるべ
  makeSignpost(3300, 150, -Math.PI / 2, '三ツ池 →')
  { makeTorii(2470, 320, Math.PI / 2) // 師岡熊野神社（西の台地の縁・homage）：鳥居＋小さな社
    const sy = heightAt(2440, 320), s = new THREE.Group()
    ...
    s.traverse((o) => { if (o.isMesh) o.castShadow = true }); s.position.set(2440, sy, 320); mergedOutline(s, 0.04); addContactShadow(s, 4); addBox(2440, 320, 2, 1.6, 0); scene.add(s) }
  for (const [hx, hz, hr] of [[2920, -60, 0.2], [2880, 20, -0.3], [3080, -70, 0.4], [3130, 40, -0.5], [3060, 110, 0.1], [2930, 130, 0.6], [3170, -130, -0.2], [2850, -150, 0.3], [3050, -170, 0.5], [2900, 200, -0.4], [3150, 180, 0.2], [2820, 80, -0.6]]) makeHouse(hx, hz, hr, [0x6a7a86, 0x7a5a48, 0x586472, 0x88603e][Math.abs(hx + hz) % 4]) // 台地の宅地（1990年代前半の建売・低層）
  { const rz = 700, river = new THREE.Mesh(new THREE.PlaneGeometry(1300, 70), waterMat); river.rotation.x = -Math.PI / 2; river.position.set(3000, heightAt(3000, rz) + 0.2, rz); scene.add(river) // 鶴見川（北の低地の縁）
    for (const dx of [-450, -250, -50, 150, 350]) makeSakura(3000 + dx, 655, 1.0) }
...
window.__build = '20260621-shishigaya-real' // ビルド識別（HTMLのみ変更時もバンドル名を変えて自動更新を効かせるため）
...
function update(dt) {
  ...
  if (mode === 'walk') {
    ...
    if (area === 'field') {
      ...
      boy.position.x = THREE.MathUtils.clamp(boy.position.x, YATO.x - 695, YATO.x + 695)
      boy.position.z = THREE.MathUtils.clamp(boy.position.z, YATO.z - 745, YATO.z + 745)
    } else { // 神社
...
function warpBoyTo(sx, sy) {
  ...
  if (area === 'field') {
    ...
    wx = THREE.MathUtils.clamp(wx, YATO.x - 695, YATO.x + 695); wz = THREE.MathUtils.clamp(wz, YATO.z - 745, YATO.z + 745)
  } else { // 神社
