...
const SUNRISE_HILL = { x: TOWN.x - 65, z: TOWN.z - 8, h: 12, w: 13, d: 13 } // 丘の頂上はマンションの北東(背面側・上り左手)。坂道はマンション正面に平行な東西の道＝東(左手)が上り・西(右手)が下り
const MANSION = { x: TOWN.x - 82, z: TOWN.z - 26 } // マンション本体＝丘の南西斜面の中腹。正面(南/-z)に平行な坂道が走り、背面(北東)が崖
...
const townNightLights = [] // 夜に灯る街のあかり（窓・街灯・自販機）。nfで点灯＝夜のエモさ
{
  ...
  makeRoadRibbon(T.x - 146, T.z - 44, T.x - 36, T.z - 37, 8) // 西(下)→東(上)。マンション前を東西に通る
  // 坂道から東(上り)＝北寺尾の町へ続く気配／西(下り)＝ビスコ・しんみせへ
  makeRoadRibbon(T.x - 36, T.z - 37, T.x - 4, T.z - 20, 8) // 東端を町の本通り(T.x,T.z)へ接続
  ...
  makeGameShop(T.x - 112, T.z - 50, 0) // 坂道を西(右手)へ下った道沿い＝ビスコ（坂の上寄り）
  ...
  makeDagashi(T.x - 138, T.z - 52, 0) // さらに西へ坂を下った坂下の交差点角＝しんみせ（確定レイアウト）
