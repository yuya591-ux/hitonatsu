...
const townNightLights = [] // 夜に灯る街のあかり（窓・街灯・自販機）。nfで点灯＝夜のエモさ
{
  ...
  function makeApartment(x, z, rot, floors, units) {
    ...
    for (let f = 0; f < floors; f++) for (let u = 0; u < units; u++) {
      ...
      if (Math.random() < 0.6) { // 物干し（布団・洗濯物）＝団地の生活感
        if (Math.random() < 0.4) { const futon = new THREE.Mesh(new THREE.BoxGeometry(1.7, 0.52, 0.14), toon([0xd2a0a0, 0x9ab0d0, 0xd0c090, 0xc8b0d0][Math.floor(Math.random() * 4)])); futon.position.set(wx, wy - 0.26, D / 2 + 0.75); g.add(futon) } // 布団を干す
...
function makeVillager(x, z, opt) {
  ...
  // 髪＝額までしっかり覆う（前傾を弱め・カバー範囲を広げて“禿げ/怖い”を解消）。前髪も付ける。
  const hair = new THREE.Mesh(new THREE.SphereGeometry(0.218, 18, 14, 0, Math.PI * 2, 0, Math.PI * 0.76), toon(opt.hair)); hair.position.y = 1.38; hair.rotation.x = -0.1; g.add(hair)
  const bangs = new THREE.Mesh(new THREE.SphereGeometry(0.2, 16, 8, 0, Math.PI * 2, Math.PI * 0.3, Math.PI * 0.22), toon(opt.hair)); bangs.position.set(0, 1.42, 0.02); bangs.rotation.x = 0.32; g.add(bangs) // 前髪（額の生え際）
