...
function heightAtYato(x, z) { // 実標高をバイリニア補間。zは反転サンプル(データ側を北=-zにしたため)。±SG.half外は縁の値で頭打ち
  ...
  if (x > 4054 && x < 4112 && z > -994 && z < -938) { const dxr = Math.abs(x - 4082), dzr = Math.abs(z + 966)
    const k = smoothstep01((23 - dxr) / 7) * smoothstep01((18 - dzr) / 8); if (k > 0) h = h * (1 - k) + 12 * k // 平地はデッキ(23x15)＋幼児プールより広く＝デッキの縁が宙に浮かない
    // プールの槽（開口部）だけ掘り下げる＝水の下に段地の地面が透けない（内側を10.4mへ）
    const dxp = Math.abs(x - 4078), dzp = Math.abs(z + 966); const kp = smoothstep01((7 - dxp) / 1.3) * smoothstep01((4 - dzp) / 1.3); if (kp > 0) h = h * (1 - kp) + 10.4 * kp }
  return h
}
...
function buildShishigaya() {
  ...
  { const cx = POOL[0], cz = POOL[1], H0 = 12, deckY = H0 + 0.12, g = new THREE.Group()
