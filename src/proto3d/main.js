...
function makeRoadRibbon(x0, z0, x1, z1, width, centerline = true, concrete = false, lift = 0) { // lift＝急斜面で地形メッシュにめり込まないよう路面を余分に持ち上げる量
  ...
  if (concrete) { // しっかりしたコンクリート舗装＝はっきり見える・地形に十分乗せて“消える/めり込む”を防ぐ
    ...
    mk(width, 0.18, 0x47483f, null)          // 舗装（濃いアスファルト調＝明るい緑とはっきり対比して“道路”と分かる）
...
const townNightLights = [] // 夜に灯る街のあかり（窓・街灯・自販機）。nfで点灯＝夜のエモさ
{
  ...
  makeRoadRibbon(T.x + 8, T.z + 22, T.x + 8, T.z + 60, 9, true, true, 0.5)    // ふもと(交差路)→中腹
  makeRoadRibbon(T.x + 8, T.z + 60, T.x + 8, T.z + 92, 9, true, true, 0.85)   // 中腹→頂上(裏山の頂・見晴らしの脇)＝急なので持ち上げ大
  makeRoadRibbon(T.x + 8, T.z + 92, T.x + 11, T.z + 148, 9, true, true, 0.85) // 頂上を越えて反対側を長く下り、山あいの奥へずっと続く
