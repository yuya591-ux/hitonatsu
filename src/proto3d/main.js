...
const townNightLights = [] // 夜に灯る街のあかり（窓・街灯・自販機）。nfで点灯＝夜のエモさ
{
  ...
  function makeGround(cx, cz) {
    const W = 32, D = 24, fy = heightAt(cx, cz)
    // 実物は芝生/原っぱで、あまり使われず夏は雑草が伸びていた（トノサマバッタの虫取り場）
    const grass = new THREE.Mesh(new THREE.PlaneGeometry(W, D), new THREE.MeshToonMaterial({ color: 0x8fa850, gradientMap: GRAD, map: watercolorTex })); grass.rotation.x = -Math.PI / 2; grass.position.set(cx, fy + 0.04, cz); grass.receiveShadow = true; scene.add(grass)
    // 伸びた夏草（30〜40cmの雑草が点々と）
    for (let i = 0; i < 60; i++) {
      const wx = cx + (Math.random() - 0.5) * (W - 2), wz = cz + (Math.random() - 0.5) * (D - 2)
      const tuft = new THREE.Mesh(new THREE.IcosahedronGeometry(0.18 + Math.random() * 0.12, 0), toon([0x7e9a44, 0x8aa850, 0x9ab85a][i % 3])); tuft.scale.set(1, 1.8 + Math.random(), 1)
      tuft.position.set(wx, fy + 0.28, wz); tuft.castShadow = true; scene.add(tuft)
      swayables.push({ obj: tuft, ph: Math.random() * 6.28, amp: 0.06 })
    }
    const lineMat = new THREE.MeshToonMaterial({ color: 0xd2d0c2, gradientMap: GRAD }) // 使われず薄れた白線
    const line = (lx, lz, lw, ld) => { const l = new THREE.Mesh(new THREE.PlaneGeometry(lw, ld), lineMat); l.rotation.x = -Math.PI / 2; l.position.set(cx + lx, fy + 0.06, cz + lz); scene.add(l) }
    line(0, -D / 2 + 1.5, W - 3, 0.25); line(0, D / 2 - 1.5, W - 3, 0.25); line(-W / 2 + 1.5, 0, 0.25, D - 3); line(W / 2 - 1.5, 0, 0.25, D - 3); line(0, 0, 0.25, D - 3) // 外枠＋センターライン
    const circ = new THREE.Mesh(new THREE.RingGeometry(2.2, 2.45, 30), lineMat); circ.rotation.x = -Math.PI / 2; circ.position.set(cx, fy + 0.06, cz); scene.add(circ) // センターサークル
    function goal(gz, dir) {
      const gg = new THREE.Group(); const gm = toon(0xf2f2ee)
      ...
      const net = new THREE.Mesh(new THREE.PlaneGeometry(7.2, 2.4), new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.16, side: THREE.DoubleSide })); net.position.set(0, 1.2, dir * 1.1); gg.add(net)
      const netT = new THREE.Mesh(new THREE.PlaneGeometry(7.2, 1.5), net.material); netT.rotation.x = Math.PI / 2 - dir * 0.5; netT.position.set(0, 2.1, dir * 0.6); gg.add(netT)
      gg.traverse((o) => { if (o.isMesh) o.castShadow = true })
      gg.position.set(cx, fy, cz + gz); mergedOutline(gg, 0.03); scene.add(gg)
    }
    goal(-D / 2 + 2, -1); goal(D / 2 - 2, 1)
    // 金網フェンス（支柱）＋ベンチ
    for (let i = 0; i <= 12; i++) for (const zz of [cz - D / 2, cz + D / 2]) { const p = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 1.8, 5), toon(0xa8a89c)); p.position.set(cx - W / 2 + i * W / 12, fy + 0.9, zz); scene.add(p) }
    for (const bz of [cz - D / 2 - 1.2, cz + D / 2 + 1.2]) { const bench = new THREE.Mesh(new THREE.BoxGeometry(4, 0.16, 0.5), toon(0x9a6a3a)); bench.position.set(cx - 6, fy + 0.45, bz); bench.castShadow = true; addOutline(bench, 0.02); scene.add(bench); for (const lx of [-1.7, 1.7]) { const leg = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.45, 0.4), toon(0x7a5230)); leg.position.set(cx - 6 + lx, fy + 0.22, bz); scene.add(leg) } }
  }
  makeGround(T.x - 82, T.z + 14)
...
function makeBoy() {
  ...
  net.position.set(0.3, 0.82, -0.1); net.rotation.set(-0.2, 0, -0.55) // 肩にかつぐ
  g.add(net)
  // 斜めがけのかばん（夏の探検バッグ）。胴に付くので歩いても自然
  const bagBody = toon(0xb98a52), bagStrap = toon(0x8a6a3a)
  const strap = new THREE.Mesh(new THREE.BoxGeometry(0.045, 0.74, 0.08), bagStrap)
  strap.position.set(0.02, 0.98, 0.2); strap.rotation.set(0.16, 0, 0.58); g.add(strap) // 左肩→右腰へ斜めに
  const satchel = new THREE.Mesh(new THREE.BoxGeometry(0.24, 0.22, 0.14), bagBody)
  satchel.position.set(0.25, 0.66, 0.2); satchel.rotation.z = 0.07; g.add(satchel)
  const flap = new THREE.Mesh(new THREE.BoxGeometry(0.26, 0.11, 0.16), toon(0xa6794a))
...
function update(dt) {
  ...
  const slen = THREE.MathUtils.clamp(1 / elev, 1, 3.0)
  const jShrink = Math.min(0.5, jumpY * 0.42) // 跳ぶと影が小さく
  const sgy = heightAt(boy.position.x, boy.position.z) + 0.05
  boyShadow.position.set(boy.position.x - sunDir.x * 0.28 * slen, sgy, boy.position.z - sunDir.z * 0.28 * slen)
  boyShadow.rotation.set(-Math.PI / 2, 0, Math.atan2(-sunDir.x, -sunDir.z)) // 影が伸びる向き
  boyShadow.scale.set(1.1 * (1 - jShrink), 1.1 * slen * (1 - jShrink), 1)
