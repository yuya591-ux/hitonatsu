...
const colliders = []
const auditProps = [] // 監査対象の小物（電柱/自販機/看板）＝道路/水/建物に立っていないか後で点検する（ユーザー要望2026-06-27）
const builtBuildings = [] // 実際に建った建物（cx,cz,w,d,ang）＝道路に乗っていないか後で監査（ユーザー指摘「道路の上に家」2026-06-27）
const roadSegs = [] // 手描きの道(makeRoadRibbon)の元の端点（x0,z0,x1,z1,w）＝道の途切れ(端点ギャップ)を後で監査（ユーザー指摘2026-06-27）
...
// 小道(下記)を挟んで両側に整然と家が並ぶ＝住宅地の通り。西列(x4132・道側=東を向く)／東列(x4170・道側=西を向く)＋通りの突き当り
for (const h of [[4132, -1012, 8, 9, 0, 2, 0], [4132, -1050, 9, 8, 0, 1, 0], [4132, -1088, 8, 9, 0, 2, 0], [4132, -1124, 9, 8, 0, 2, 0],
  [4170, -1028, 9, 8, 0, 2, 0], [4170, -1066, 8, 9, 0, 1, 0], [4170, -1104, 9, 8, 0, 2, 0],
  [4124, -1150, 8, 9, 0, 2, 0], [4162, -1148, 9, 8, 0, 2, 0]]) SG.buildings.push(h)
for (const r of SG.roads) for (const p of r.p) p[1] = -p[1]
// 北門の住宅地の通り（土の小道）＝プール北東の出口あたりから北へ、家並みのあいだを抜ける。z反転後のゲーム座標で直接push＝マスク(歩ける)・路面描画・道上の木伐採をそのまま通す
...
function heightAtYato(x, z) { // 実標高をバイリニア補間。zは反転サンプル(データ側を北=-zにしたため)。±SG.half外は縁の値で頭打ち
  ...
  // 三ツ池公園の一帯だけ鉛直強調を弱める＝池が深い崖の底に見える問題を解消し、二つ池のように緩やかな岸に（ユーザー要望2026-06-25）
  let exag = 0.8; { const dd = Math.hypot(x - 3950, z + 790), wmt = smoothstep01((560 - dd) / 240); exag = 0.8 - 0.55 * wmt } // 中心320m以内で0.25・560mで0.8へ戻す
  let h = Math.max(2, e + (e - 18) * exag) // 鉛直強調(基準18m)：DEM5A(5mメッシュ)が均す急な切土/坂を、丘谷の“形”はそのままに急勾配へ。位置は不変。三ツ池域だけ緩やか
  // やまゆりホーム入口前の広場＝斜面(西27m〜東17m)を水平な段地(H0=24m)に。建物が山に食い込まず・夏祭りの平らな広場に。縁はsmoothstepで地形へなだらかに（ユーザー指定ポリゴン 2885,-489〜2922,-479 / 2026-06-25）
  if (x > 2873 && x < 2933 && z > -528 && z < -460) { const dxr = Math.abs(x - 2903), dzr = Math.abs(z + 494)
    const k = smoothstep01((29 - dxr) / 10) * smoothstep01((33 - dzr) / 10); if (k > 0) h = h * (1 - k) + 24 * k }
  // 三ツ池公園のプール＝下の池の北東、北門ぎわのほぼ平らな地(約11m)を水平な段地(H0=12m)にならす。実在の位置に忠実(北門徒歩3分・ユーザー要望2026-06-25・公式サイト調査)
  if (x > 4054 && x < 4112 && z > -994 && z < -938) { const dxr = Math.abs(x - 4082), dzr = Math.abs(z + 966)
    const k = smoothstep01((23 - dxr) / 7) * smoothstep01((18 - dzr) / 8); if (k > 0) h = h * (1 - k) + 14 * k // 平地はデッキ(23x15)＋幼児プールより広く＝デッキの縁が宙に浮かない（地形をゆるくした分H0を14へ）
    // プールの槽（開口部）だけ掘り下げる＝水の下に段地の地面が透けない（内側を12.4mへ）
    const dxp = Math.abs(x - 4078), dzp = Math.abs(z + 966); const kp = smoothstep01((7 - dxp) / 1.3) * smoothstep01((4 - dzp) / 1.3); if (kp > 0) h = h * (1 - kp) + 12.4 * kp }
  // 中の池の西岸ぎわ(3790,-741)のDEMスパイク＝乾いた地面なのに標高約4.5(周囲18〜21)の謎の深穴。三ツ池域の鉛直強調が孤立した低値をさらに掘り下げていた→周囲の高さへ埋め戻す（Math.maxで“持ち上げるだけ”＝池/滑り台/周囲の地形は一切下げない・ユーザー指摘2026-06-26）
  { const dxr = Math.abs(x - 3790), dzr = Math.abs(z + 741), k = smoothstep01((9 - dxr) / 7) * smoothstep01((9 - dzr) / 7); if (k > 0) h = Math.max(h, h * (1 - k) + 19 * k) }
  return h
}
...
function makeRoadRibbon(x0, z0, x1, z1, width, centerline = true, concrete = false, lift = 0) { // lift＝急斜面で地形メッシュにめり込まないよう路面を余分に持ち上げる量
  if (x0 > 2200 || x1 > 2200) roadSegs.push([x0, z0, x1, z1, width]) // ヤトの手描き道の端点を登録（途切れ監査用・元の端点＝ext拡張前）
...
function buildBonOdori(ox, oy, oz, grp) {
  ...
  for (let i = 0; i < 16; i++) { const a = (i / 16) * Math.PI * 2; const pan = new THREE.Mesh(new THREE.BoxGeometry(0.58, 0.7, 0.04), toon(i % 2 ? 0xf2eee2 : 0xc83a3a)); pan.position.set(Math.sin(a) * (S + 0.28), 1.28, Math.cos(a) * (S + 0.28)); pan.rotation.y = a; yag.add(pan) } // 紅白幕
  ...
  const roof = new THREE.Mesh(new THREE.ConeGeometry(S * 1.95, 1.0, 4), toon(0x9a3a32)); roof.rotation.y = Math.PI / 4; roof.position.y = 3.55; yag.add(roof) // 屋根
  ...
  for (let i = 0; i < NP; i++) {
    ...
    for (let k = 1; k <= N; k++) { const t = k / (N + 1), lx = x0 + (x1 - x0) * t, lz = z0 + (z1 - z0) * t, ly = y0 + (y1 - y0) * t - Math.sin(t * Math.PI) * 0.5
      const body = new THREE.Mesh(new THREE.SphereGeometry(0.16, 10, 8), toon(0xd9483a)); body.scale.y = 1.3; body.position.set(lx, ly, lz); bonOdori.add(body)
      const glow = new THREE.Mesh(new THREE.SphereGeometry(0.3, 12, 10), new THREE.MeshBasicMaterial({ color: 0xffa84e, fog: false, transparent: true, opacity: 0, blending: THREE.AdditiveBlending, depthWrite: false })); glow.scale.y = 1.3; glow.position.set(lx, ly, lz); bonOdori.add(glow)
...
function buildShishigaya() {
  ...
  const NAMED = [
    ...
    [2903, 507, 'yamayuri', 'やまゆりホーム', 30], // 特養・昭和58(1983)開園・獅子ケ谷2-15-18＝二ツ池を見下ろす丘。入口前の広場(game 2903,-484)の南に建ち、玄関が広場へ向く。広場で夏祭り（2日目）。建物足元は段地化で山に食い込まない（2026-06-25 ユーザー📍ピン）
  ...
  // 看板・店先の向き＝固定点でなく“最寄りの道”を向く。z反転後の世界で看板が裏向き(=鏡文字)・店が逆向きになっていた不具合の修正（ユーザー指摘2026-06-24・全エリア対象）。道沿いに正面が向き、歩く側から文字が正しく読める
  const roadPtsForFace = []
  for (const rd of SG.roads) { const p = rd.p; for (let k = 0; k < p.length - 1; k++) { const a = p[k], b = p[k + 1], dx = b[0] - a[0], dz = b[1] - a[1], l = Math.hypot(dx, dz) || 1; for (let t = 0; t < l; t += 6) roadPtsForFace.push([a[0] + dx * t / l, a[1] + dz * t / l]) } }
  const faceRoad = (cx, cz) => { let bx = 3008, bz = -8, bd = 1e18; for (const pt of roadPtsForFace) { const dd = (pt[0] - cx) * (pt[0] - cx) + (pt[1] - cz) * (pt[1] - cz); if (dd < bd) { bd = dd; bx = pt[0]; bz = pt[1] } } return Math.atan2(bx - cx, bz - cz) } // 最寄りの道の方を向く角（店先・看板が道に正対）
  ...
  SG.buildings.forEach(([cx, cz, w, d, ang, lv, tc], bi) => {
    ...
    builtBuildings.push([cx, cz, w, d, ang]) // 実際に建つ建物を登録（道路重なり監査用）
    addBox(cx, cz, w / 2, d / 2, -ang, 0.3) // 当たり判定＝見た目の壁(L/+ang)に正対させる。当たり判定の解決は-ang相当なので-angで登録（以前は鏡像回転で建物の横に見えない壁が出ていた・2026-06-25）
  ...
  { const grp = new THREE.Group(); grp.name = 'landmarks2'; scene.add(grp)
    ...
    const buildShop = (cx, cz, w, d, floors, wallCol, name, bg) => { const gB = gmin4(cx, cz, w, d), gT = gmax4(cx, cz, w, d), slope = Math.min(8, gT - gB), h = slope + floors * 3, ry = faceRoad(cx, cz), fwd = [Math.sin(ry), Math.cos(ry)]
    ...
    const buildConbini = (cx, cz, name) => { const gB = gmin4(cx, cz, 14, 11), gT = gmax4(cx, cz, 14, 11), slope = Math.min(5, gT - gB), ry = faceRoad(cx, cz), fwd = [Math.sin(ry), Math.cos(ry)], side = [Math.cos(ry), -Math.sin(ry)], W = 13, D = 9, H = slope + 3.6
    ...
    const buildCarShop = (cx, cz, name) => { const gB = gmin4(cx, cz, 12, 9), gT = gmax4(cx, cz, 12, 9), slope = Math.min(5, gT - gB), ry = faceRoad(cx, cz), fwd = [Math.sin(ry), Math.cos(ry)], side = [Math.cos(ry), -Math.sin(ry)], W = 11, D = 8, H = slope + 3.4
      const f = (lx, lz) => [cx + fwd[0] * lz + side[0] * lx, cz + fwd[1] * lz + side[1] * lx] // 店のローカル(lx=左右,lz=前後)→世界
      const [pkx, pkz] = f(0, 9); groundPatch(grp, pkx, pkz, 22, 13, 0x97978f) // 前面のアスファルト展示場
      grp.add(mk(new THREE.BoxGeometry(W, H, D), toon(0xe9e3d2), cx, gB + H / 2, cz, ry, true)) // ショールームの箱（クリーム）
      grp.add(mk(new THREE.BoxGeometry(W + 0.5, 0.6, D + 0.5), toon(0x6b5a48), cx, gB + H + 0.2, cz, ry, true)) // 陸屋根の庇
      const cglass = new THREE.MeshBasicMaterial({ color: 0xeef0ea, transparent: true, opacity: 0.86, side: THREE.DoubleSide }) // 明るいショールームのガラス
      grp.add(mk(new THREE.PlaneGeometry(W * 0.9, H - 1.0), cglass, cx + fwd[0] * (D / 2 + 0.05), gB + (H - 1.0) / 2 + 0.1, cz + fwd[1] * (D / 2 + 0.05), ry)) // 一面ガラス
      ...
      { const ay = ply + 3.5, by = pry2 + 3.5, N = 14, fcol = [0xd8542e, 0xf1c40f, 0x2e8b57, 0x2f6db0, 0xe07a2e], flagGeo = new THREE.PlaneGeometry(0.32, 0.44)
        for (let i = 0; i < N; i++) { const t = (i + 0.5) / N, fx = plx + (prx - plx) * t, fz = plz + (prz - plz) * t, fy = ay + (by - ay) * t - Math.sin(t * Math.PI) * 0.5; const m = new THREE.Mesh(flagGeo, new THREE.MeshToonMaterial({ color: fcol[i % fcol.length], gradientMap: GRAD, side: THREE.DoubleSide })); m.position.set(fx, fy - 0.22, fz); m.rotation.y = ry; grp.add(m) } } // 小旗の連なり（たわみ付き）
      for (const s of [-1, 1]) { const [nx, nz] = f(s * 6.5, D / 2 + 2.2), ny = heightAtYato(nx, nz); grp.add(mk(new THREE.CylinderGeometry(0.05, 0.05, 3.2, 6), toon(0xcfcabd), nx, ny + 1.6, nz, 0, true)); grp.add(mk(new THREE.PlaneGeometry(0.5, 2.2), new THREE.MeshToonMaterial({ color: s > 0 ? 0xd8542e : 0x1b3c6e, gradientMap: GRAD, side: THREE.DoubleSide }), nx + side[0] * 0.3, ny + 1.9, nz + side[1] * 0.3, ry)) } // のぼり2本
      addBox(cx, cz, W / 2, D / 2, ry, 0.3) }
    // ───── 町のスーパー（1980〜90年代＝マミート等）＝大きな箱＋赤い電照看板＋一面ガラス＋中央の自動ドア＋前面の駐車場＋のぼり＋カート置き場。※実在ブランドのロゴ/色は不使用＝架空名の看板のみ ─────
    const buildSuper = (cx, cz, name) => { const gB = gmin4(cx, cz, 22, 16), gT = gmax4(cx, cz, 22, 16), slope = Math.min(6, gT - gB), ry = faceRoad(cx, cz), fwd = [Math.sin(ry), Math.cos(ry)], side = [Math.cos(ry), -Math.sin(ry)], W = 20, D = 14, H = slope + 5.0
      const f = (lx, lz) => [cx + fwd[0] * lz + side[0] * lx, cz + fwd[1] * lz + side[1] * lx]
      const [pkx, pkz] = f(0, 13); groundPatch(grp, pkx, pkz, 28, 18, 0x9a9a96) // 前面の駐車場（アスファルト）
      grp.add(mk(new THREE.BoxGeometry(W, H, D), toon(0xe6e0d2), cx, gB + H / 2, cz, ry, true)) // 大きな箱（クリーム）
      grp.add(mk(new THREE.BoxGeometry(W + 0.7, 0.9, D + 0.7), toon(0xcdc8ba), cx, gB + H + 0.35, cz, ry, true)) // 陸屋根パラペット
      const glass = new THREE.MeshBasicMaterial({ color: 0xf2efe2, transparent: true, opacity: 0.9, side: THREE.DoubleSide }) // 明るい店内
      grp.add(mk(new THREE.PlaneGeometry(W * 0.94, H - 2.2), glass, cx + fwd[0] * (D / 2 + 0.05), gB + (H - 2.2) / 2 + 0.2, cz + fwd[1] * (D / 2 + 0.05), ry)) // 一面ガラス
      const mull = toon(0xbfc4c8); for (let i = -3; i <= 3; i++) { const [mx, mz] = f(i * 2.7, D / 2 + 0.07); grp.add(mk(new THREE.BoxGeometry(0.12, H - 2.2, 0.12), mull, mx, gB + (H - 2.2) / 2 + 0.2, mz, ry)) } // ガラスの方立
      grp.add(mk(new THREE.BoxGeometry(W + 0.5, 1.7, 0.3), new THREE.MeshBasicMaterial({ map: signTex(name, '#b5462f', '#fff8e8') }), cx + fwd[0] * (D / 2 + 0.2), gB + H - 1.0, cz + fwd[1] * (D / 2 + 0.2), ry)) // 正面の赤い電照看板帯
      grp.add(mk(new THREE.BoxGeometry(4.0, 2.6, 0.2), toon(0x7a8088), cx + fwd[0] * (D / 2 + 0.1), gB + 1.3, cz + fwd[1] * (D / 2 + 0.1), ry)) // 中央の自動ドア枠
      grp.add(mk(new THREE.PlaneGeometry(3.4, 2.3), glass, cx + fwd[0] * (D / 2 + 0.2), gB + 1.25, cz + fwd[1] * (D / 2 + 0.2), ry)) // 自動ドアのガラス
    ...
    const buildYamayuri = (cx, cz, name) => {
      const PX = 2903, PZ = -484, PY = heightAtYato(PX, PZ) // 入口前の広場＝ユーザー指定ポリゴンの重心(game 2903,-484)。段地化で水平・本体(2903,-507)の北に隣接＝玄関が広場へ開く。夏祭りの輪/屋台/ぼんぼりはこの平らな広場に（開催日は2日目）
    ...
    const buildMise = (cx, cz, name, kind, forceRy) => { const gB = gmin4(cx, cz, 9, 8), gT = gmax4(cx, cz, 9, 8), slope = Math.min(6, gT - gB), ry = forceRy != null ? forceRy : faceRoad(cx, cz), fwd = [Math.sin(ry), Math.cos(ry)], side = [Math.cos(ry), -Math.sin(ry)], W = 8, D = 7, H1 = 2.7, H2 = 2.4, H = slope + H1 + H2 // forceRyで店先の向きを明示指定（faceRoadが裏の道を拾う時の上書き）
      ...
      { const awHex = kind === 'eat' ? '#a8463a' : kind === 'liquor' ? '#3a5f92' : kind === 'green' ? '#3f8240' : kind === 'flower' ? '#4f9070' : kind === 'tobacco' ? '#6f6f78' : '#52725a' // 縞の色（店の種類で）
        const awnTex = (() => { const c = document.createElement('canvas'); c.width = 64; c.height = 32; const x = c.getContext('2d'); for (let i = 0; i < 8; i++) { x.fillStyle = i % 2 ? '#f4efe2' : awHex; x.fillRect(i * 8, 0, 8, 32) } const t = new THREE.CanvasTexture(c); t.wrapS = t.wrapT = THREE.RepeatWrapping; t.repeat.set(Math.round(W / 1.1), 1); return t })() // 日除けテントの縞（縦縞・店色と生成り）
        const awnMat = new THREE.MeshToonMaterial({ color: 0xffffff, map: awnTex, gradientMap: GRAD, side: THREE.DoubleSide })
        const [agx, agz] = f(0, D / 2 + 0.05); const awnG = new THREE.Group(); awnG.position.set(agx, eaveY + 0.22, agz); awnG.rotation.y = ry; grp.add(awnG) // 店先に縞の日除けテント＝昭和の商店街（E9・2026-06-25）
    ...
    const buildKomyoji = (cx, cz, name) => { const gy = gmin4(cx, cz, 24, 24)
      ...
      groundPatch(grp, cx, cz + 1, 26, 32, 0xcfc8b6) // 境内（玉砂利＝寺の敷地だと分かる地面色）
      // 山門（表門・瓦の切妻門・薬医門ふう）front=南(cz+14)
      { const tz = cz + 14, ty = heightAtYato(cx, tz)
        for (const sx of [-2.4, 2.4]) grp.add(mk(new THREE.CylinderGeometry(0.22, 0.24, 3.6, 8), woodD, cx + sx, ty + 1.8, tz, 0, true)) // 主柱2本
        for (const sx of [-2.4, 2.4]) grp.add(mk(new THREE.CylinderGeometry(0.16, 0.18, 3.2, 8), woodD, cx + sx, ty + 1.6, tz - 1.8, 0, true)) // 控え柱
        grp.add(mk(new THREE.BoxGeometry(6.2, 0.5, 0.5), woodD, cx, ty + 3.7, tz)) // 冠木
        for (const sz of [-1, 1]) mr(new THREE.BoxGeometry(6.8, 0.18, 2.2), tile, cx, ty + 4.4, tz + sz * 0.9, sz * 0.5) // 切妻の瓦屋根2斜面
        grp.add(mk(new THREE.BoxGeometry(7.0, 0.22, 0.3), tile, cx, ty + 4.95, tz)) // 棟
        addBox(cx - 2.4, tz, 0.5, 0.5, 0, 0.3); addBox(cx + 2.4, tz, 0.5, 0.5, 0, 0.3) } // 柱だけ当たり判定（中央は通れる）
      grp.add(mk(new THREE.BoxGeometry(3.0, 0.1, 13), toon(0xc7c3b6), cx, gy + 0.07, cz + 6, 0, true)) // 参道
      for (const lz of [cz + 11, cz + 6]) { lantern(cx - 3.2, lz); lantern(cx + 3.2, lz) } // 参道の石灯籠
      // 本堂（瓦の入母屋・大堂＋向拝）中央(cz-3)
      { const bx = cx, bz = cz - 3, by = heightAtYato(bx, bz)
        grp.add(mk(new THREE.BoxGeometry(15, 0.6, 12.5), stoneL, bx, by + 0.3, bz, 0, true)) // 基壇
        grp.add(mk(new THREE.BoxGeometry(13.5, 4.0, 11), wall, bx, by + 2.6, bz, 0, true)) // 本堂の身舎
        const roof = mr(new THREE.ConeGeometry(12.5, 5.0, 4), tile, bx, by + 7.3, bz, 0, Math.PI / 4); roof.scale.set(1.05, 1, 0.84) // 瓦の入母屋大屋根（寄棟で近似）
        grp.add(mk(new THREE.BoxGeometry(8, 0.55, 0.6), tile, bx, by + 9.6, bz)) // 大棟
        for (const sx of [-4, 4]) grp.add(mk(new THREE.BoxGeometry(0.5, 0.7, 0.5), toon(0x3c4046), bx + sx, by + 9.9, bz, 0, true)) // 鬼瓦（両端）
        // 向拝（正面の張り出し階段屋根）
        for (const sx of [-2.2, 2.2]) grp.add(mk(new THREE.CylinderGeometry(0.16, 0.18, 3.0, 8), woodR, bx + sx, by + 2.0, bz + 6.0, 0, true)); mr(new THREE.BoxGeometry(5.2, 0.16, 2.6), tile, bx, by + 4.0, bz + 6.4, 0.42); grp.add(mk(new THREE.BoxGeometry(5.2, 0.4, 1.0), stoneL, bx, by + 0.45, bz + 6.6, 0, true)) // 向拝の柱＋瓦＋階段
        grp.add(mk(new THREE.BoxGeometry(3.0, 2.4, 0.3), woodR, bx, by + 1.8, bz + 5.5, 0, true)) // 正面の扉（朱）
        addBox(bx, bz, 7, 5.8, 0, 0.4) } // 本堂の当たり判定
      // 庫裡（東・切妻の瓦屋根の居住棟）
      { const kx = cx + 12, kz = cz - 6, ky = heightAtYato(kx, kz); grp.add(mk(new THREE.BoxGeometry(7, 3.4, 8), wall, kx, ky + 1.7, kz, 0, true)); for (const sz of [-1, 1]) mr(new THREE.BoxGeometry(7.6, 0.18, 5.0), tile, kx, ky + 4.0, kz + sz * 2.1, sz * 0.4); grp.add(mk(new THREE.BoxGeometry(7.8, 0.2, 0.3), tile, kx, ky + 4.7, kz)); addBox(kx, kz, 3.6, 4.1, 0, 0.3) } // 庫裡＋切妻瓦
      // 鐘楼（西前・袴腰なしの簡素な4本柱＋宝形屋根＋梵鐘）
      { const sx2 = cx - 11, sz2 = cz + 5, sy = heightAtYato(sx2, sz2); grp.add(mk(new THREE.BoxGeometry(3.4, 0.5, 3.4), stoneL, sx2, sy + 0.25, sz2, 0, true)); for (const [dx, dz] of [[-1.3, -1.3], [1.3, -1.3], [-1.3, 1.3], [1.3, 1.3]]) grp.add(mk(new THREE.CylinderGeometry(0.13, 0.13, 3.0, 6), woodD, sx2 + dx, sy + 2.0, sz2 + dz, 0, true)); mr(new THREE.ConeGeometry(2.9, 2.0, 4), tile, sx2, sy + 4.4, sz2, 0, Math.PI / 4); grp.add(mk(new THREE.CylinderGeometry(0.4, 0.34, 1.4, 8), toon(0x6a6256), sx2, sy + 3.0, sz2, 0, true)); addBox(sx2, sz2, 1.8, 1.8, 0, 0.3) } // 鐘楼＋梵鐘
    ...
    const buildHongakuji = (cx, cz, name) => { const gy = gmin4(cx, cz, 24, 24)
      ...
      groundPatch(grp, cx, cz + 1, 26, 32, 0xcfc8b6) // 境内（玉砂利）
      // 山門（瓦の切妻・薬医門）front=南(cz+14)
      { const tz = cz + 14, ty = heightAtYato(cx, tz)
        for (const sx of [-2.4, 2.4]) grp.add(mk(new THREE.CylinderGeometry(0.22, 0.24, 3.6, 8), woodD, cx + sx, ty + 1.8, tz, 0, true))
        for (const sx of [-2.4, 2.4]) grp.add(mk(new THREE.CylinderGeometry(0.16, 0.18, 3.2, 8), woodD, cx + sx, ty + 1.6, tz - 1.8, 0, true))
        grp.add(mk(new THREE.BoxGeometry(6.2, 0.5, 0.5), woodD, cx, ty + 3.7, tz))
        for (const sz of [-1, 1]) mr(new THREE.BoxGeometry(6.8, 0.18, 2.2), tile, cx, ty + 4.4, tz + sz * 0.9, sz * 0.5)
        grp.add(mk(new THREE.BoxGeometry(7.0, 0.22, 0.3), tile, cx, ty + 4.95, tz))
        addBox(cx - 2.4, tz, 0.5, 0.5, 0, 0.3); addBox(cx + 2.4, tz, 0.5, 0.5, 0, 0.3) }
      grp.add(mk(new THREE.BoxGeometry(3.0, 0.1, 13), toon(0xc7c3b6), cx, gy + 0.07, cz + 6, 0, true)) // 参道
      for (const lz of [cz + 11, cz + 6]) { lantern(cx - 3.2, lz); lantern(cx + 3.2, lz) }
      // 本堂（瓦の入母屋・大堂＋向拝）中央(cz-3)。光明寺より少し横長＋落ち着いた色
      { const bx = cx, bz = cz - 3, by = heightAtYato(bx, bz)
        grp.add(mk(new THREE.BoxGeometry(15.5, 0.6, 12), stoneL, bx, by + 0.3, bz, 0, true))
        grp.add(mk(new THREE.BoxGeometry(14, 4.0, 10.5), wall, bx, by + 2.6, bz, 0, true))
        const roof = mr(new THREE.ConeGeometry(13, 5.0, 4), tile, bx, by + 7.3, bz, 0, Math.PI / 4); roof.scale.set(1.06, 1, 0.82)
        grp.add(mk(new THREE.BoxGeometry(8.5, 0.55, 0.6), tile, bx, by + 9.6, bz))
        for (const sx of [-4.2, 4.2]) grp.add(mk(new THREE.BoxGeometry(0.5, 0.7, 0.5), toon(0x3c4046), bx + sx, by + 9.9, bz, 0, true))
        for (const sx of [-2.2, 2.2]) grp.add(mk(new THREE.CylinderGeometry(0.16, 0.18, 3.0, 8), woodR, bx + sx, by + 2.0, bz + 5.7, 0, true)); mr(new THREE.BoxGeometry(5.2, 0.16, 2.6), tile, bx, by + 4.0, bz + 6.1, 0.42); grp.add(mk(new THREE.BoxGeometry(5.4, 0.4, 1.0), stoneL, bx, by + 0.45, bz + 6.3, 0, true))
        grp.add(mk(new THREE.BoxGeometry(3.0, 2.4, 0.3), woodR, bx, by + 1.8, bz + 5.3, 0, true))
        addBox(bx, bz, 7.2, 5.5, 0, 0.4) }
      // 観音堂（受賞作・六角の宝形堂＝本覺寺の象徴）西前(cx-9, cz+5)
      { const ox = cx - 9, oz = cz + 5, oy = heightAtYato(ox, oz)
        grp.add(mk(new THREE.CylinderGeometry(2.6, 2.7, 0.5, 6), stoneL, ox, oy + 0.25, oz, 0, true)) // 基壇（六角）
        grp.add(mk(new THREE.CylinderGeometry(2.2, 2.2, 2.6, 6), wall, ox, oy + 1.8, oz, 0, true)) // 六角の堂
        grp.add(mk(new THREE.ConeGeometry(2.9, 2.2, 6), copper, ox, oy + 4.2, oz, 0, true)) // 宝形の銅板屋根（六角・緑青）
        grp.add(mk(new THREE.CylinderGeometry(0.08, 0.08, 1.3, 6), gold, ox, oy + 5.6, oz)); grp.add(mk(new THREE.SphereGeometry(0.22, 8, 6), gold, ox, oy + 6.3, oz, 0, true)) // 相輪・宝珠
        grp.add(mk(new THREE.BoxGeometry(1.0, 1.8, 0.2), woodR, ox, oy + 1.4, oz + 2.15, 0, true)) // 正面の扉
        addCollider(ox, oz, 2.6) }
      // 庫裡（東）
      { const kx = cx + 12, kz = cz - 6, ky = heightAtYato(kx, kz); grp.add(mk(new THREE.BoxGeometry(7, 3.4, 8), wall, kx, ky + 1.7, kz, 0, true)); for (const sz of [-1, 1]) mr(new THREE.BoxGeometry(7.6, 0.18, 5.0), tile, kx, ky + 4.0, kz + sz * 2.1, sz * 0.4); grp.add(mk(new THREE.BoxGeometry(7.8, 0.2, 0.3), tile, kx, ky + 4.7, kz)); addBox(kx, kz, 3.6, 4.1, 0, 0.3) }
      // 鐘楼（西前）
      { const sx2 = cx - 11, sz2 = cz + 11, sy = heightAtYato(sx2, sz2); grp.add(mk(new THREE.BoxGeometry(3.4, 0.5, 3.4), stoneL, sx2, sy + 0.25, sz2, 0, true)); for (const [dx, dz] of [[-1.3, -1.3], [1.3, -1.3], [-1.3, 1.3], [1.3, 1.3]]) grp.add(mk(new THREE.CylinderGeometry(0.13, 0.13, 3.0, 6), woodD, sx2 + dx, sy + 2.0, sz2 + dz, 0, true)); mr(new THREE.ConeGeometry(2.9, 2.0, 4), tile, sx2, sy + 4.4, sz2, 0, Math.PI / 4); grp.add(mk(new THREE.CylinderGeometry(0.4, 0.34, 1.4, 8), toon(0x6a6256), sx2, sy + 3.0, sz2, 0, true)); addBox(sx2, sz2, 1.8, 1.8, 0, 0.3) }
      // 宝篋印塔＋石仏（本覺寺の森観音霊園のしるし）東前(cx+9, cz+8)
      { const hx = cx + 9, hz = cz + 8, hy = heightAtYato(hx, hz); grp.add(mk(new THREE.BoxGeometry(1.0, 0.4, 1.0), stoneL, hx, hy + 0.2, hz, 0, true)); grp.add(mk(new THREE.BoxGeometry(0.7, 1.0, 0.7), stone, hx, hy + 0.9, hz, 0, true)); grp.add(mk(new THREE.BoxGeometry(0.9, 0.3, 0.9), stoneL, hx, hy + 1.55, hz, 0, true)); for (const sx of [-0.4, 0.4]) for (const sz of [-0.4, 0.4]) grp.add(mk(new THREE.ConeGeometry(0.12, 0.45, 4), stoneL, hx + sx, hy + 1.9, hz + sz, 0, true)); grp.add(mk(new THREE.CylinderGeometry(0.06, 0.06, 0.95, 6), stoneL, hx, hy + 2.15, hz)); grp.add(mk(new THREE.SphereGeometry(0.13, 8, 6), stoneL, hx, hy + 2.7, hz, 0, true)) } // 宝篋印塔（隅飾突起＋相輪）
    ...
    const buildMyokoji = (cx, cz, name) => { const gy = gmin4(cx, cz, 24, 24)
      ...
      { const tz = cz + 14, ty = heightAtYato(cx, tz)
        ...
        grp.add(mk(new THREE.BoxGeometry(7.0, 0.22, 0.3), tile, cx, ty + 4.95, tz))
        addBox(cx - 2.4, tz, 0.5, 0.5, 0, 0.3); addBox(cx + 2.4, tz, 0.5, 0.5, 0, 0.3) }
      // 題目塔（南無妙法蓮華経の石塔＝日蓮宗の象徴）参道入口の東寄り(cx+4, cz+11)
      { const dx = cx + 4, dz = cz + 11, dyy = heightAtYato(dx, dz)
        grp.add(mk(new THREE.BoxGeometry(1.2, 0.5, 1.2), stoneL, dx, dyy + 0.25, dz, 0, true)) // 基礎
        grp.add(mk(new THREE.BoxGeometry(0.72, 4.4, 0.72), stone, dx, dyy + 2.7, dz, 0, true)) // 塔身
        grp.add(mk(new THREE.PlaneGeometry(0.64, 3.9), new THREE.MeshBasicMaterial({ map: daimokuTex }), dx, dyy + 2.8, dz + 0.37)) // 正面の題目
        grp.add(mk(new THREE.BoxGeometry(0.95, 0.3, 0.95), stoneL, dx, dyy + 5.05, dz, 0, true)) // 笠石
        addCollider(dx, dz, 0.8) }
      grp.add(mk(new THREE.BoxGeometry(3.0, 0.1, 13), toon(0xc7c3b6), cx, gy + 0.07, cz + 6, 0, true)) // 参道
      for (const lz of [cz + 8]) { lantern(cx - 3.2, lz); lantern(cx + 3.2, lz) }
      // 本堂（瓦の入母屋・大堂＋向拝・朱の扉に題目額）中央(cz-3)
      { const bx = cx, bz = cz - 3, by = heightAtYato(bx, bz)
        grp.add(mk(new THREE.BoxGeometry(15, 0.6, 12.5), stoneL, bx, by + 0.3, bz, 0, true))
        grp.add(mk(new THREE.BoxGeometry(13.5, 4.2, 11), wall, bx, by + 2.7, bz, 0, true))
        const roof = mr(new THREE.ConeGeometry(12.6, 5.2, 4), tile, bx, by + 7.6, bz, 0, Math.PI / 4); roof.scale.set(1.04, 1, 0.85)
        grp.add(mk(new THREE.BoxGeometry(8, 0.55, 0.6), tile, bx, by + 10.0, bz))
        for (const sx of [-4, 4]) grp.add(mk(new THREE.BoxGeometry(0.5, 0.7, 0.5), toon(0x3c4046), bx + sx, by + 10.3, bz, 0, true))
        for (const sx of [-2.2, 2.2]) grp.add(mk(new THREE.CylinderGeometry(0.16, 0.18, 3.2, 8), woodR, bx + sx, by + 2.1, bz + 6.0, 0, true)); mr(new THREE.BoxGeometry(5.2, 0.16, 2.6), tile, bx, by + 4.2, bz + 6.4, 0.42); grp.add(mk(new THREE.BoxGeometry(5.4, 0.4, 1.0), stoneL, bx, by + 0.45, bz + 6.6, 0, true))
        grp.add(mk(new THREE.BoxGeometry(3.2, 2.5, 0.3), woodR, bx, by + 1.85, bz + 5.6, 0, true)) // 正面の扉（朱）
        grp.add(mk(new THREE.PlaneGeometry(1.0, 0.5), new THREE.MeshBasicMaterial({ map: signTex('妙法', '#2a2a28', '#e8c84a'), side: THREE.DoubleSide }), bx, by + 3.55, bz + 5.62)) // 扉上の題目額
        addBox(bx, bz, 7, 5.8, 0, 0.4) }
      // 庫裡（東）
      { const kx = cx + 12, kz = cz - 6, ky = heightAtYato(kx, kz); grp.add(mk(new THREE.BoxGeometry(7, 3.4, 8), wall, kx, ky + 1.7, kz, 0, true)); for (const sz of [-1, 1]) mr(new THREE.BoxGeometry(7.6, 0.18, 5.0), tile, kx, ky + 4.0, kz + sz * 2.1, sz * 0.4); grp.add(mk(new THREE.BoxGeometry(7.8, 0.2, 0.3), tile, kx, ky + 4.7, kz)); addBox(kx, kz, 3.6, 4.1, 0, 0.3) }
      // 鐘楼（西前）
      { const sx2 = cx - 11, sz2 = cz + 5, sy = heightAtYato(sx2, sz2); grp.add(mk(new THREE.BoxGeometry(3.4, 0.5, 3.4), stoneL, sx2, sy + 0.25, sz2, 0, true)); for (const [dx, dz] of [[-1.3, -1.3], [1.3, -1.3], [-1.3, 1.3], [1.3, 1.3]]) grp.add(mk(new THREE.CylinderGeometry(0.13, 0.13, 3.0, 6), woodD, sx2 + dx, sy + 2.0, sz2 + dz, 0, true)); mr(new THREE.ConeGeometry(2.9, 2.0, 4), tile, sx2, sy + 4.4, sz2, 0, Math.PI / 4); grp.add(mk(new THREE.CylinderGeometry(0.4, 0.34, 1.4, 8), toon(0x6a6256), sx2, sy + 3.0, sz2, 0, true)); addBox(sx2, sz2, 1.8, 1.8, 0, 0.3) }
      // 日蓮聖人像（立像・錫杖）参道脇(cx-4, cz+9)
    ...
    const buildInari = (cx, cz, name) => { const gy = heightAtYato(cx, cz)
      const vermil = toon(0xc14230), vermilD = toon(0x9c3221), white = toon(0xece6da), foxC = toon(0xe7e1d2), red = toon(0xb52f22), stone = toon(0xa8a59a), woodD = toon(0x6a5236), gold = toon(0xb89a4a), roofC = toon(0x4c4742)
      const mr = (geo, mat, x, y, z, rx, ry) => { const m = new THREE.Mesh(geo, mat); m.position.set(x, y, z); if (rx) m.rotation.x = rx; if (ry) m.rotation.y = ry; m.castShadow = m.receiveShadow = true; grp.add(m); return m }
      groundPatch(grp, cx, cz, 9, 20, 0xc6bea7) // 境内（玉砂利）
      // 朱の連鳥居（明神鳥居が並ぶ＝稲荷の象徴）front=南。cz+11→cz+3.8 に5基
      for (let i = 0; i < 5; i++) { const tz = cz + 11 - i * 1.8, ty = heightAtYato(cx, tz), tw = 1.1
        for (const sx of [-tw, tw]) grp.add(mk(new THREE.CylinderGeometry(0.1, 0.12, 2.7, 8), vermil, cx + sx, ty + 1.35, tz, 0, true)) // 柱
        grp.add(mk(new THREE.BoxGeometry(tw * 2 + 0.7, 0.18, 0.3), vermilD, cx, ty + 2.7, tz, 0, true)) // 島木
        grp.add(mk(new THREE.BoxGeometry(tw * 2 + 1.0, 0.16, 0.36), vermil, cx, ty + 2.9, tz, 0, true)) // 笠木
        grp.add(mk(new THREE.BoxGeometry(tw * 2 + 0.2, 0.13, 0.2), vermil, cx, ty + 2.15, tz)) } // 貫
      // お狐2体（社前・赤前掛け・参道を向く）
      const fox = (fx, fz) => { const fy = heightAtYato(fx, fz); grp.add(mk(new THREE.BoxGeometry(0.42, 0.85, 0.42), stone, fx, fy + 0.42, fz, 0, true)) // 台座
        const g = new THREE.Group(); g.position.set(fx, fy + 0.85, fz); grp.add(g); const ga = (geo, mat, x, y, z, rx) => { const m = new THREE.Mesh(geo, mat); m.position.set(x, y, z); if (rx) m.rotation.x = rx; m.castShadow = true; g.add(m) }
        ga(new THREE.BoxGeometry(0.22, 0.55, 0.32), foxC, 0, 0.28, 0); ga(new THREE.SphereGeometry(0.15, 8, 6), foxC, 0, 0.66, 0.1) // 胴＋頭
        for (const sx of [-0.07, 0.07]) ga(new THREE.ConeGeometry(0.05, 0.16, 4), foxC, sx, 0.82, 0.08) // 耳
        ga(new THREE.CylinderGeometry(0.04, 0.09, 0.55, 5), foxC, 0, 0.34, -0.2, -0.5); ga(new THREE.BoxGeometry(0.26, 0.2, 0.04), red, 0, 0.36, 0.18) } // 尾＋赤い前掛け
      fox(cx - 1.7, cz + 1); fox(cx + 1.7, cz + 1)
      // 朱塗りの社（流造・千木鰹木）北(cz-3.5)
      { const sx2 = cx, sz2 = cz - 3.5, sy = heightAtYato(sx2, sz2)
        for (const [dx, dz] of [[-1.4, -0.8], [1.4, -0.8], [-1.4, 0.8], [1.4, 0.8]]) grp.add(mk(new THREE.CylinderGeometry(0.09, 0.09, 1.2, 6), vermilD, sx2 + dx, sy + 0.6, sz2 + dz)) // 高床の柱
        grp.add(mk(new THREE.BoxGeometry(3.4, 0.3, 2.4), vermilD, sx2, sy + 1.2, sz2, 0, true)) // 床
        grp.add(mk(new THREE.BoxGeometry(2.6, 1.7, 1.8), vermil, sx2, sy + 2.2, sz2, 0, true)) // 身舎（朱）
        grp.add(mk(new THREE.BoxGeometry(1.2, 1.3, 0.2), white, sx2, sy + 1.8, sz2 + 0.95)) // 扉（白）
        mr(new THREE.BoxGeometry(3.4, 0.16, 1.5), roofC, sx2, sy + 3.35, sz2 - 0.55, -0.5) // 背の短い屋根
        mr(new THREE.BoxGeometry(3.4, 0.16, 2.6), roofC, sx2, sy + 3.1, sz2 + 1.05, 0.42) // 流造＝前に長く流れる屋根
        grp.add(mk(new THREE.BoxGeometry(3.5, 0.16, 0.2), woodD, sx2, sy + 3.7, sz2 - 0.2)) // 棟
        for (let i = 0; i < 3; i++) mr(new THREE.CylinderGeometry(0.07, 0.07, 0.9, 8), gold, sx2 - 0.8 + i * 0.8, sy + 3.8, sz2 - 0.2, Math.PI / 2) // 鰹木
        for (const s of [-1.4, 1.4]) for (const d of [-1, 1]) mr(new THREE.BoxGeometry(0.09, 1.1, 0.09), woodD, sx2 + s, sy + 3.95, sz2 - 0.2, d * 0.5) // 千木
        grp.add(mk(new THREE.BoxGeometry(0.9, 0.6, 0.45), woodD, sx2, sy + 0.6, sz2 + 1.6, 0, true)) // 賽銭箱
        addCollider(sx2, sz2, 2.0) }
      // 朱の幟（奉納のぼり）2本
      for (const sx of [-3, 3]) { const ny = heightAtYato(cx + sx, cz + 8); grp.add(mk(new THREE.CylinderGeometry(0.05, 0.05, 3.6, 6), woodD, cx + sx, ny + 1.8, cz + 8, 0, true)); grp.add(mk(new THREE.PlaneGeometry(0.5, 2.6), new THREE.MeshToonMaterial({ color: 0xb83020, gradientMap: GRAD, side: THREE.DoubleSide }), cx + sx + 0.3, ny + 2.1, cz + 8)) }
      precinctFence(grp, cx, cz, 9, 20, 0xb0432e, 0.95, 's') // 朱の玉垣（敷地境界・南＝鳥居側を開ける）
      signOn(cx, cz + 13, 4, gy, 2.4, name, '#9c3221') }
    const buildSchoolDetailed = (cx, cz, name) => { // 獅子ヶ谷小学校＝ユーザー指定ピン(2026-06-22)に忠実：裏門(西)→広場+小池→校舎(北西)→階段→校庭(東の一段高い平地・約34.5m)。プールは実位置。全部地面に沿う＝浮かない・歩ける
      ...
      { const yc = 3124, yz = -186
        ...
        for (const [bx, bh] of [[3104, 1.1], [3106.2, 1.45]]) { const by = heightAtYato(bx, -214) // 鉄棒（高さ違い2本・西寄り）
          for (const sx of [-0.75, 0.75]) { const p = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, bh, 6), toon(0x8a9aa2)); p.position.set(bx + sx, by + bh / 2, -214); p.castShadow = true; grp.add(p) }
          const bar = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 1.6, 6), toon(0xb0bcc2)); bar.rotation.z = Math.PI / 2; bar.position.set(bx, by + bh, -214); grp.add(bar) }
    ...
    const buildApt = (cx, cz, w, d, floors, name) => { const gB = gmin4(cx, cz, w, d), slope = Math.min(10, gmax4(cx, cz, w, d) - gB), h = slope + floors * 3, ry = faceRoad(cx, cz), co = Math.cos(ry), si = Math.sin(ry), hw = w / 2, hd = d / 2 // 当時の中層マンション＝バルコニー面＋陸屋根＋名前看板（バルコニー/名前看板が道に正対＝看板が裏向き鏡文字にならない・2026-06-24）
      const L = (lx, ly, lz) => [cx + lx * co + lz * si, gB + ly, cz - lx * si + lz * co], base = [[-hw, -hd], [hw, -hd], [hw, hd], [-hw, hd]], vv = [], uvv = [], ix = []; let o = 0 // ★回転はmk(rotation.y=ry)と同じ向きに統一＝壁と陸屋根/看板/当たり判定がズレない・道に正対（以前は逆回転で長い棟の屋根が横にずれていた）
  ...
  const pondInfo = SG.waters.filter((w) => w.p.length >= 3).map((w) => { let mnx = 1e9, mxx = -1e9, mnz = 1e9, mxz = -1e9, cx = 0, cz = 0; for (const q of w.p) { cx += q[0]; cz += q[1]; if (q[0] < mnx) mnx = q[0]; if (q[0] > mxx) mxx = q[0]; if (q[1] < mnz) mnz = q[1]; if (q[1] > mxz) mxz = q[1] } cx /= w.p.length; cz /= w.p.length; return { w, cx, cz, area: (mxx - mnx) * (mxz - mnz), r: Math.max(mxx - mnx, mxz - mnz) / 2, minExt: Math.min(mxx - mnx, mxz - mnz) } }).sort((a, b) => b.area - a.area)
  // D2：実在する池(二ツ池/谷戸の池/川)を「釣り場」として登録＝谷戸で釣りができる。岸からの最短距離で判定するため多角形の点も持つ。小さすぎる水たまり/溝は除く。
  //   br＝中心から最も遠い頂点までの距離（細長い池は重心が偏り bbox半径だと岸が粗ふるいから漏れるため、必ず全頂点を含む半径にする）
  for (const pi of pondInfo) { if (pi.minExt < 8) continue; let br = 0; for (const q of pi.w.p) { const d = Math.hypot(q[0] - pi.cx, q[1] - pi.cz); if (d > br) br = d } YATO_PONDS.push({ cx: pi.cx, cz: pi.cz, br, p: pi.w.p, yato: true }) }
  ...
  // ── 雨上がりの水たまり：平らな道に点々と。夕立→虹のあと路面に空が映る（屈指のエモい瞬間）。全部を1メッシュにマージ＝1ドロー。2026-06-24 ──
  { const pud = [], m4 = new THREE.Matrix4(), seen = [], sv = new THREE.Vector3()
    for (const rd of SG.roads) { if (pud.length >= 14) break; const p = rd.p
      for (let k = 0; k < p.length - 1 && pud.length < 14; k++) { const x = (p[k][0] + p[k + 1][0]) / 2, z = (p[k][1] + p[k + 1][1]) / 2
        if (Math.hypot(x - 3010, z + 120) > 250) continue // 歩く核の近くだけ
        const sl = Math.abs(heightAtYato(x + 3, z) - heightAtYato(x - 3, z)) + Math.abs(heightAtYato(x, z + 3) - heightAtYato(x, z - 3)); if (sl > 1.2) continue // 平らな所だけ（坂に水はたまらない）
        let near = false; for (const s of seen) { if (Math.hypot(x - s[0], z - s[1]) < 24) { near = true; break } } if (near) continue
        seen.push([x, z]); const r = 1.0 + Math.random() * 1.5, cg = new THREE.CircleGeometry(r, 16); cg.rotateX(-Math.PI / 2)
  ...
  { const mhTex = (() => { const c = document.createElement('canvas'); c.width = c.height = 48; const x = c.getContext('2d'); x.fillStyle = '#54585e'; x.beginPath(); x.arc(24, 24, 23, 0, 6.283); x.fill(); x.strokeStyle = '#3c4046'; x.lineWidth = 2; for (const r of [19, 13, 7]) { x.beginPath(); x.arc(24, 24, r, 0, 6.283); x.stroke() } for (let a = 0; a < 8; a++) { x.beginPath(); x.moveTo(24, 24); x.lineTo(24 + Math.cos(a / 8 * 6.283) * 22, 24 + Math.sin(a / 8 * 6.283) * 22); x.stroke() } return new THREE.CanvasTexture(c) })()
    ...
    if (mhP.length) { const mI = new THREE.InstancedMesh(new THREE.CircleGeometry(0.42, 14), new THREE.MeshBasicMaterial({ map: mhTex, side: THREE.DoubleSide }), mhP.length); const m4 = new THREE.Matrix4(), qx = new THREE.Quaternion().setFromEuler(new THREE.Euler(-Math.PI / 2, 0, 0)), qs = new THREE.Quaternion(), es = new THREE.Euler(), qb = new THREE.Quaternion(), s = new THREE.Vector3(1, 1, 1), col = new THREE.Color()
      mhP.forEach(([x, z], i) => { es.set(0, 0, Math.random() * 6.283); qs.setFromEuler(es); qb.copy(qx).multiply(qs) // 蓋ごとに向きをばらつかせる（盤面で回す＝平らは保つ）＝一様な並びを解消
        m4.compose(new THREE.Vector3(x, heightAtYato(x, z) + 0.06, z), qb, s); mI.setMatrixAt(i, m4); const t = Math.random(); col.setRGB(0.84 + t * 0.2, 0.84 + t * 0.18, 0.82 + t * 0.16); mI.setColorAt(i, col) }) // 鉄蓋ごとに微妙な明暗（錆/濡れ）
      if (mI.instanceColor) mI.instanceColor.needsUpdate = true; mI.layers.set(1); scene.add(mI) } // マンホール（平・インク除外）
  ...
  if (tp.length) {
    ...
    canMat.onBeforeCompile = (sh) => { sh.uniforms.uTime = { value: 0 }; sh.uniforms.uWind = { value: 0.5 }
      sh.vertexShader = sh.vertexShader.replace('#include <common>', '#include <common>\nuniform float uTime;\nuniform float uWind;')
        .replace('#include <begin_vertex>', `#include <begin_vertex>
        float tph = (instanceMatrix[3].x - instanceMatrix[3].z) * 0.11;
  ...
  { const cx = POOL[0], cz = POOL[1], H0 = 14, deckY = H0 + 0.12, g = new THREE.Group()
    const deckMat = toon(0xc7c9bd), coping = toon(0xeae6da), waterMat = new THREE.MeshToonMaterial({ color: 0x4fb6d6, gradientMap: GRAD, transparent: true, opacity: 0.82 }), tile = toon(0x8fcfe0), tileD = toon(0x6bb0cc), fenceMat = toon(0xb6bcc0), red = toon(0xd8663a), woodM = toon(0x9a7b4e)
    ...
    const DW = 23, DD = 15, pw = 14, pd = 7 // デッキ全体・25mプール本体（縮尺）
    // デッキ（プール開口のまわりの歩道＝4枚の枠）
    add(new THREE.BoxGeometry(DW, 0.24, (DD - pd) / 2), deckMat, cx, deckY, cz - (DD + pd) / 4) // 北
    add(new THREE.BoxGeometry(DW, 0.24, (DD - pd) / 2), deckMat, cx, deckY, cz + (DD + pd) / 4) // 南
    add(new THREE.BoxGeometry((DW - pw) / 2, 0.24, pd), deckMat, cx - (DW + pw) / 4, deckY, cz) // 西
    add(new THREE.BoxGeometry((DW - pw) / 2, 0.24, pd), deckMat, cx + (DW + pw) / 4, deckY, cz) // 東
    // デッキのへりの腰壁（擁壁）＝段地のへりの段差を隠す（デッキが宙に浮かない）
    for (const [bw, bd, ox, oz] of [[DW + 0.4, 0.5, 0, -DD / 2], [DW + 0.4, 0.5, 0, DD / 2], [0.5, DD + 0.4, -DW / 2, 0], [0.5, DD + 0.4, DW / 2, 0]]) add(new THREE.BoxGeometry(bw, 2.4, bd), deckMat, cx + ox, deckY - 1.2, cz + oz)
    // プールの縁石（白いコーピング）
    for (const [bw, bd, ox, oz] of [[pw + 0.6, pd * 0 + 0.32, 0, -(pd / 2 + 0.16)], [pw + 0.6, 0.32, 0, pd / 2 + 0.16], [0.32, pd + 0.6, -(pw / 2 + 0.16), 0], [0.32, pd + 0.6, pw / 2 + 0.16, 0]]) add(new THREE.BoxGeometry(bw, 0.16, bd), coping, cx + ox, deckY + 0.08, cz + oz)
    // 水面＋槽（内壁4面＋底）＝開口部の掘り下げ地形を隠す
    add(new THREE.BoxGeometry(pw, 0.06, pd), waterMat, cx, deckY - 0.25, cz)
    const wallY = deckY - 0.78
    add(new THREE.BoxGeometry(pw + 0.2, 1.55, 0.22), tile, cx, wallY, cz - pd / 2); add(new THREE.BoxGeometry(pw + 0.2, 1.55, 0.22), tile, cx, wallY, cz + pd / 2)
    add(new THREE.BoxGeometry(0.22, 1.55, pd), tile, cx - pw / 2, wallY, cz); add(new THREE.BoxGeometry(0.22, 1.55, pd), tile, cx + pw / 2, wallY, cz)
    add(new THREE.BoxGeometry(pw, 0.2, pd), tileD, cx, deckY - 1.5, cz) // 槽の底
    for (let l = 1; l < 4; l++) add(new THREE.BoxGeometry(pw, 0.05, 0.06), coping, cx, deckY - 0.2, cz - pd / 2 + (pd / 4) * l) // コースロープ（白い浮き玉のレーン）
    // まわりの金網フェンス（柱＋上下2段の桟・南の中央だけ入口で開ける）
    const fy = deckY + 0.02, perim = []
    for (let t = 0; t <= DW; t += 2.6) { perim.push([cx - DW / 2 + t, cz - DD / 2]); if (Math.abs(cx - DW / 2 + t - cx) > 2) perim.push([cx - DW / 2 + t, cz + DD / 2]) } // 北辺・南辺(中央2mは入口で抜く)
    for (let t = 2.6; t < DD; t += 2.6) { perim.push([cx - DW / 2, cz - DD / 2 + t]); perim.push([cx + DW / 2, cz - DD / 2 + t]) } // 東西辺
    for (const [px, pz] of perim) add(new THREE.CylinderGeometry(0.05, 0.05, 1.5, 6), fenceMat, px, fy + 0.75, pz)
    for (const [ax, az, bx, bz] of [[cx - DW / 2, cz - DD / 2, cx + DW / 2, cz - DD / 2], [cx - DW / 2, cz + DD / 2, cx - 1.5, cz + DD / 2], [cx + 1.5, cz + DD / 2, cx + DW / 2, cz + DD / 2], [cx - DW / 2, cz - DD / 2, cx - DW / 2, cz + DD / 2], [cx + DW / 2, cz - DD / 2, cx + DW / 2, cz + DD / 2]]) { const mx = (ax + bx) / 2, mz = (az + bz) / 2, len = Math.hypot(bx - ax, bz - az), ry = Math.atan2(bx - ax, bz - az); for (const yy of [1.4, 0.7]) add(new THREE.BoxGeometry(0.04, 0.05, len), fenceMat, mx, fy + yy, mz, ry) } // 上下2段の桟（入口で南辺を分割）
    // 監視台（高い椅子）＝北西の角
    { const lx = cx - DW / 2 + 2, lz = cz - 1.4; for (const [ox, oz] of [[-0.4, -0.4], [0.4, -0.4], [-0.4, 0.4], [0.4, 0.4]]) add(new THREE.CylinderGeometry(0.05, 0.05, 2.3, 6), woodM, lx + ox, deckY + 1.15, lz + oz); add(new THREE.BoxGeometry(1.1, 0.1, 0.9), woodM, lx, deckY + 2.3, lz); add(new THREE.BoxGeometry(1.1, 0.7, 0.1), woodM, lx, deckY + 2.65, lz - 0.4); add(new THREE.CylinderGeometry(0.06, 0.06, 1.7, 6), red, lx, deckY + 3.3, lz) } // パラソルの柱
...
const yatoBugs = [] // 獅子ヶ谷の生き物（とんぼ・蝶）＝池/田の上に。area非依存で常時アニメ（update参照）
{
  ...
  const addTombo = (cx, cz, col, dusk) => { const g = new THREE.Group(); const body = new THREE.Mesh(new THREE.CylinderGeometry(0.035, 0.02, 1.0, 5), toon(col || 0x4a7ab2)); body.rotation.z = Math.PI / 2; g.add(body); const w = []; for (const [sx, sz] of [[0.1, 0.26], [0.1, -0.26], [-0.1, 0.26], [-0.1, -0.26]]) { const wing = new THREE.Mesh(new THREE.PlaneGeometry(0.48, 0.16), wingMat()); wing.position.set(sx, 0.02, sz); wing.rotation.x = -Math.PI / 2; g.add(wing); w.push(wing) } g.position.set(cx, heightAtYato(cx, cz) + 1.5, cz); scene.add(g); yatoBugs.push({ obj: g, cx, cz, sp: 0.5 + Math.random() * 0.4, ph: Math.random() * 6.28, r: 2.5 + Math.random() * 2, kind: 'tombo', w, h: dusk ? 1.2 + Math.random() * 0.6 : 1.5, dusk }) } // col=胴の色／dusk=夕暮れだけ出る赤とんぼ
  const addCho = (cx, cz, col) => { const g = new THREE.Group(); const wMat = new THREE.MeshToonMaterial({ color: col, gradientMap: GRAD, side: THREE.DoubleSide, transparent: true, opacity: 0.9 }) // 羽はわずかに透ける＝のっぺりした板に見えない
    const body = new THREE.Mesh(new THREE.CylinderGeometry(0.016, 0.01, 0.3, 5), toon(0x2e251c)); g.add(body) // 細い黒っぽい胴
    const wgL = new THREE.PlaneGeometry(0.2, 0.28); wgL.translate(-0.1, 0, 0); const wl = new THREE.Mesh(wgL, wMat); g.add(wl) // 内側の縁を回転軸に＝本物のように開閉する
    const wgR = new THREE.PlaneGeometry(0.2, 0.28); wgR.translate(0.1, 0, 0); const wr = new THREE.Mesh(wgR, wMat); g.add(wr)
    g.position.set(cx, heightAtYato(cx, cz) + 1.3, cz); scene.add(g); yatoBugs.push({ obj: g, cx, cz, sp: 0.6 + Math.random() * 0.5, ph: Math.random() * 6.28, r: 1.5 + Math.random() * 1.5, kind: 'cho', wl, wr, h: 1.3 }) }
  ...
  let cn = 0; for (const g of SG.greens) { if ((g.kind !== 'farm' && g.kind !== 'park') || cn >= 16) continue; let cx = 0, cz = 0; for (const q of g.p) { cx += q[0]; cz += q[1] } cx /= g.p.length; cz /= g.p.length; addCho(cx, cz, [0xf0e060, 0xf4f1e6, 0xdf9440][cn % 3]); cn++ } // 田/公園の蝶＝モンキ(黄)/モンシロ(白)/タテハ(橙)＝実在の夏の蝶
  // 核の散歩道(サンライズ〜小学校〜二ツ池)＋住宅街にも蝶・とんぼを足す＝歩く所に夏の生気（ユーザー要望2026-06-23）
  const coreCho = [[3010, -28, 0xf0e060], [3038, -88, 0xf4f1e6], [3072, -150, 0xdf9440], [3028, -208, 0xf0e060], [3052, -300, 0xf4f1e6], [3060, -430, 0xdf9440], [3120, -108, 0xf0e060], [3182, -70, 0xf4f1e6], [3242, -120, 0xdf9440], [3150, -198, 0xf0e060], [2980, 38, 0xf4f1e6], [3100, -262, 0xdf9440], [3210, -40, 0xf0e060], [3280, -150, 0xf4f1e6], [2960, -90, 0xdf9440], [3160, -330, 0xf0e060]]
  for (const [x, z, col] of coreCho) { if (heightAtYato(x, z) >= 2) addCho(x, z, col) }
  for (const [x, z] of [[3015, -120], [3045, -262], [3055, -480], [3030, -360], [3090, -540]]) { if (heightAtYato(x, z) >= 2) addTombo(x, z) } // 谷の道沿いのとんぼ
  // 赤とんぼ：夏の終わりの夕暮れ、青田の上をすうっと群れて舞う＝晩夏の田舎の夕方そのもの（夕暮れだけ出る）。青田(farm)の上に2匹ずつ
  { let rc = 0; for (const grn of SG.greens) { if (grn.kind !== 'farm' || rc >= 9) continue
    let cx = 0, cz = 0; for (const q of grn.p) { cx += q[0]; cz += q[1] } cx /= grn.p.length; cz /= grn.p.length
    if (heightAtYato(cx, cz) < 2) continue
    const red = [0xc23a2c, 0xcf5a36, 0xb83a30][rc % 3]; addTombo(cx, cz, red, true); addTombo(cx + 6, cz - 5, red, true); rc++ } }
}
// ── 案山子（かかし）：青田に立つ、夏の田舎の原風景。十字の骨組み＋古シャツ＋菅笠＋へのへのもへじ風の顔。風でゆらり（2026-06-26）──
function makeScarecrow(x, z, rot) {
  const g = new THREE.Group(), gy = heightAtYato(x, z)
  const wood = toon(0x8a6b46), cloth = toon([0x5f7488, 0x8a6a5a, 0x6a8a64, 0x9a8a56, 0x99565a][Math.floor(Math.random() * 5)]) // 古シャツの色をばらす
  const post = new THREE.Mesh(new THREE.CylinderGeometry(0.035, 0.05, 1.7, 6), wood); post.position.y = 0.85; g.add(post) // 縦の支柱（地中から立つ）
  const arm = new THREE.Mesh(new THREE.CylinderGeometry(0.028, 0.028, 1.12, 6), wood); arm.rotation.z = Math.PI / 2; arm.position.y = 1.2; g.add(arm) // 横木＝腕
  const body = new THREE.Mesh(new THREE.CylinderGeometry(0.16, 0.21, 0.52, 10), cloth); body.scale.set(1, 1, 0.58); body.position.y = 1.0; g.add(body) // 胴＝古シャツ（前後に平たい）
  for (const sx of [-1, 1]) { const sleeve = new THREE.Mesh(new THREE.CylinderGeometry(0.055, 0.04, 0.46, 6), cloth); sleeve.position.set(sx * 0.4, 1.04, 0); sleeve.rotation.z = sx * 0.42; g.add(sleeve) } // 袖（横木に沿って垂れる）
  for (const sx of [-1, 1]) { const hand = new THREE.Mesh(new THREE.ConeGeometry(0.05, 0.2, 5), toon(0xcdb678)); hand.position.set(sx * 0.56, 0.96, 0); hand.rotation.x = Math.PI; g.add(hand) } // 藁の手（横木の先）
  const head = new THREE.Mesh(new THREE.SphereGeometry(0.13, 12, 10), toon(0xe8ddc2)); head.position.y = 1.42; g.add(head) // 頭＝白い布玉
  const ink = new THREE.MeshBasicMaterial({ color: 0x2a2218 })
  for (const ex of [-0.05, 0.05]) { const eye = new THREE.Mesh(new THREE.SphereGeometry(0.015, 6, 6), ink); eye.position.set(ex, 1.445, 0.115); g.add(eye) } // 目（点）
  const smile = Math.random() < 0.6; const mouth = new THREE.Mesh(new THREE.TorusGeometry(0.022, 0.005, 6, 10, Math.PI), ink); mouth.position.set(0, smile ? 1.4 : 1.41, 0.118); mouth.rotation.z = smile ? Math.PI : 0; g.add(mouth) // 口（にっこり/への字）
  const kasa = new THREE.Mesh(new THREE.ConeGeometry(0.21, 0.15, 14), toon(0xd6bc7a)); kasa.position.y = 1.55; g.add(kasa) // 菅笠（三角の藁帽子）
  for (const m of [post, body, head, kasa]) addOutline(m, 0.02)
  g.traverse((o) => { if (o.isMesh) o.castShadow = true })
  g.position.set(x, gy, z); g.rotation.y = rot || 0; g.userData = { scarecrow: true }; scene.add(g)
  addContactShadow(g, 0.5)
  swayables.push({ obj: g, ph: Math.random() * 6.28, amp: 0.02 }) // 風でゆらり（根元から傾く）
  return g
}
{ const inW = (x, z) => SG.waters.some((q) => q.p && q.p.length >= 3 && pip(x, z, q.p)) // 水判定（inWaterはこの時点で未定義なのでpipで自前に＝モジュール評価エラー防止）
  let sc = 0; const placed = []; for (const grn of SG.greens) { if (grn.kind !== 'farm' || sc >= 7) continue // 青田のいくつかに1体ずつ
    let cx = 0, cz = 0, mnx = 1e9, mxx = -1e9, mnz = 1e9, mxz = -1e9; for (const q of grn.p) { cx += q[0]; cz += q[1]; mnx = Math.min(mnx, q[0]); mxx = Math.max(mxx, q[0]); mnz = Math.min(mnz, q[1]); mxz = Math.max(mxz, q[1]) } cx /= grn.p.length; cz /= grn.p.length
    if ((mxx - mnx) * (mxz - mnz) < 120) continue // 小さすぎる田は置かない
    ...
    makeJizo(spot[0], spot[1], Math.atan2(tx - spot[0], tz - spot[1])); placed.push(spot) } } // 道(目標点)の方を向く
{
  ...
  // ── C⑬（ユーザー承認2026-06-27）：尾根(z-200)から北寺尾の集落(z-300)へ下る“裸の草地の斜面”を作り込む。
  //   主要道の両脇に雑木林・下草・路傍の生活感を足して「夏の峠道」に。すべて heightAt 接地の純粋追加＝地形は不変。
  //   ※道(中心x857〜870/幅8m)を避けて両側へオフセット配置。家は斜面のゆるい下側(集落寄り)だけに。
  {
    // 雑木林（道の西側＝下り谷側／東側＝山側）。サイズに変化をつけて“わさっと”茂らせる
    for (const [x, z, s] of [
      [844, -207, 1.10], [849, -217, 0.95], [841, -227, 1.18], [847, -237, 1.02], [843, -249, 1.08], [850, -259, 0.92], [845, -269, 1.12],
      [875, -210, 1.00], [881, -219, 1.16], [877, -231, 0.94], [884, -241, 1.10], [879, -253, 1.04], [886, -263, 0.98], [882, -274, 1.06],
    ]) makeTree(x, z, s)
    // 下草・低木（木立のあいだに散らす＝斜面に緑の厚み）
    for (const [x, z, s] of [
      [852, -212, 0.9], [846, -232, 1.0], [854, -252, 0.85], [848, -276, 0.95],
      [872, -224, 0.9], [880, -246, 1.0], [874, -266, 0.85], [884, -284, 0.95],
    ]) makeBush(x, z, s)
    // 路傍の電柱（昭和の峠道）＝道の東縁のすぐ外側に等間隔。道(半幅4m)を避けて約6mオフセット
    for (const [x, z] of [[866, -214], [868, -244], [872, -272]]) makePole(x, z)
    // 道のカーブの路傍に車地蔵（峠道の祠。道(東)の方を向く）
    makeJizo(850, -239, Math.PI / 2)
    // 斜面のゆるい下側（集落寄り）に古い家を2軒＝道へ正対。生垣で庭先を囲う
    makeHouse(842, -282, Math.PI / 2, 0x9c6f52)   // 西側の家＝東(道)向き
    makeHedge(836, -276, 836, -290)                // その庭先の生垣
    makeHouse(888, -280, -Math.PI / 2, 0x8a7d63)  // 東側の家＝西(道)向き
  }
  ...
  function makeApartment(x, z, rot, floors, units) {
    ...
    for (let f = 0; f < floors; f++) for (let u = 0; u < units; u++) {
      ...
      if (Math.random() < 0.86) { // 物干し（布団・洗濯物）＝団地の生活感（出現率0.72→0.86＝あの頃の住宅地の干し物をもっと・ユーザー要望2026-06-24）
  ...
  function makeSchool(cx, cz) {
    ...
    // ── 盆踊りの会場は獅子ヶ谷小学校の校庭へ移設（buildBonOdori）。旧プロト町のこの会場は無効化して残置（プレイヤーは獅子ヶ谷で過ごすため）2026-06-24 ──
    if (false) {
...
// 獅子ヶ谷の夏の彩り：開けた庭/参道脇にひまわりの小群れ（D8・2026-06-25）。道/水/建物に重ならない既知の開けた場所のみ・実機スクショで確認
for (const [hx, hz] of [[2380, -654], [2382, -657], [2378, -651], [2960, -322], [2957, -319], [2963, -325]]) makeSunflower(hx, hz)
...
function fixProps() {
  let moved = 0
  for (const pr of auditProps) { if (!pr.g || pr.x < 2200) continue // ヤト(主舞台)の小物だけ＝町/旧フィールドの電線つき電柱に触れない（配線が浮くため）
    const ground = (X, Z) => heightAtYato(X, Z)
    const bad = (X, Z) => npcInWater(X, Z) || onYatoRoadCore(X, Z) || deepInBuilding(X, Z)
    if (!bad(pr.x, pr.z)) continue
    let found = null
    for (let rr = 2; rr <= 18 && !found; rr += 1.5) for (let a = 0; a < 6.2832; a += 0.4) { const ex = pr.x + Math.cos(a) * rr, ez = pr.z + Math.sin(a) * rr; if (!bad(ex, ez)) { found = { x: ex, z: ez }; break } }
    if (found) { const dy = pr.g.position.y - ground(pr.x, pr.z); pr.g.position.set(found.x, ground(found.x, found.z) + dy, found.z); pr.x = found.x; pr.z = found.z; moved++ } // y方向の据え付けオフセットは保つ
  }
  // 店番も「建物に深く埋まる」なら店先の路傍へ逃がす（店先＝壁際は正常なので深く埋まる者だけ）
  if (typeof shopkeepers !== 'undefined') for (const p of shopkeepers) { if (!p.position || p.position.x < 2200 || !deepInBuilding(p.position.x, p.position.z)) continue
    let found = null
    for (let rr = 1.5; rr <= 12 && !found; rr += 1) for (let a = 0; a < 6.2832; a += 0.4) { const ex = p.position.x + Math.cos(a) * rr, ez = p.position.z + Math.sin(a) * rr; if (!deepInBuilding(ex, ez) && !npcInWater(ex, ez) && !onYatoRoadCore(ex, ez)) { found = { x: ex, z: ez }; break } }
    if (found) { const dy = p.position.y - heightAtYato(p.position.x, p.position.z); p.position.set(found.x, heightAtYato(found.x, found.z) + dy, found.z); moved++ }
  }
  return moved
}
fixProps() // 構築直後に一度だけ点検＝建物に埋まった自販機/看板/電柱を路傍へ逃がす（電線つきの旧フィールド電柱は埋まっていないので動かない）
...
addChatPair(3050, -20, 0.4)  // 第三公園のブランコぎわ（公園に必ず人を）
...
function playTempleBell() { // 遠くの寺の鐘（梵鐘・入相の鐘）＝夕暮れに谷へ響く。非整数倍音を長く減衰＋distantバスの残響で“遠い夢”の音
  if (!audioStarted) return
  try {
    const ctx = listener.context, t0 = ctx.currentTime + 0.05, out = getDistantOut(), f0 = 148 + Math.random() * 14
    for (const [mul, amp, dur] of [[1, 0.5, 7.5], [2.0, 0.3, 6.0], [2.76, 0.2, 4.8], [5.4, 0.1, 3.0]]) { const o = ctx.createOscillator(); o.type = 'sine'; o.frequency.value = f0 * mul
      const g = ctx.createGain(); g.gain.setValueAtTime(0.0001, t0); g.gain.exponentialRampToValueAtTime(amp * 0.16, t0 + 0.012); g.gain.exponentialRampToValueAtTime(0.0001, t0 + dur)
      o.connect(g); g.connect(out); o.start(t0); o.stop(t0 + dur + 0.1) } // 胴の余韻（倍音を非整数で重ねて“ゴーン”）
    const n = ctx.createBufferSource(); n.buffer = getNoise(); const nb = ctx.createBiquadFilter(); nb.type = 'bandpass'; nb.frequency.value = 300
    const ng = ctx.createGain(); ng.gain.setValueAtTime(0.05, t0); ng.gain.exponentialRampToValueAtTime(0.0001, t0 + 0.12); n.connect(nb); nb.connect(ng); ng.connect(out); n.start(t0); n.stop(t0 + 0.14) // 撞木の当たり
  } catch (e) {}
}
...
const thunderheads = []
const _cloudCamQ = new THREE.Quaternion() // 雲の擬似法線ライティング用：太陽方向をカメラのビュー空間へ変換する一時クォータニオン
...
const puffTex = (() => {
  ...
  for (let j = 0; j < S; j++) for (let i = 0; i < S; i++) {
    ...
    let a = r < 0.58 ? 1.0 : Math.max(0, 1 - (r - 0.58) / 0.42)      // 芯はr<0.58までベタ不透明＝各パフが「丸い塊」としてくっきり読める（白飛びの霧化を防ぐ）
    const edge = Math.max(0, Math.min(1, (r - 0.5) / 0.5))           // 0=芯 1=外縁
    const n = vn(i / S * 4.5 + 1.3, j / S * 4.5 + 2.1) * 0.6 + vn(i / S * 10.0, j / S * 10.0) * 0.28
    a *= 1.0 - edge * (1.0 - (0.28 + 0.78 * n))                      // 外縁だけをノイズでふわっと溶かす（芯は密のまま）＝もくもくのギザギザ輪郭
...
const cloudMat = new THREE.ShaderMaterial({
  ...
  vertexShader: `attribute vec3 iPos; attribute float iSize; attribute float iY; attribute float iSeed;
    ...
    void main(){
      vUv = uv; vY = iY;
      vLit = clamp(dot(normalize(iPos + vec3(0.0, 3.0, 0.0)), normalize(sunDir)) * 0.5 + 0.5, 0.0, 1.0); // 雲スケールの受光/陰＝太陽側のパフは明・反対側は暗（雲全体の大きな立体）
      vec4 mv = modelViewMatrix * vec4(iPos, 1.0);
      float c = cos(iSeed), s = sin(iSeed);                          // 板ごとに少し回す＝同じ絵の繰り返しを隠す
      vec2 q = vec2(position.x * c - position.y * s, position.x * s + position.y * c);
      vSph = q;                                                      // 回転後のquad座標∈[-1,1]＝各パフを「丸い塊」に見せる擬似ヘミ球面のxy（フラグメントでnz=√(1-r²)）
      mv.xy += q * iSize;
      gl_Position = projectionMatrix * mv;
    }`,
  // ★もくもく改善（3役エージェント協議2026-06-27）：パフ板1枚ごとに擬似球面法線を与えper-pixelライティング＝各ローブが太陽に丸く受光し下が陰る＝カリフラワー状の立体。陰は“暗く”でなく“青く”・受光側に暖色リム（過去の「怖い」を回避）。テクスチャ/ジオメトリ/ドローコール増なし＝モバイル負荷ほぼゼロ。
  fragmentShader: `uniform sampler2D map; uniform float opacity; uniform vec3 topCol; uniform vec3 botCol; uniform vec3 sunCol; uniform vec3 sunDirView;
    varying vec2 vUv; varying float vY; varying float vLit; varying vec2 vSph;
    void main(){
      float a = texture2D(map, vUv).a * opacity;
      if (a < 0.045) discard;                                            // 外縁の薄い羽根を捨てる＝オーバードロー削減＋輪郭をクリスプに（もくもくのフチ）
      // 疑似ヘミ球面の法線＝この板を「丸い塊」に見せる核心。中心が手前(+z)・縁が寝る
      float r2 = dot(vSph, vSph);
      float nz = sqrt(max(0.0, 1.0 - r2));
      vec3 N = vec3(vSph, nz);                                           // ビュー空間のヘミ球面法線
...
function buildCloudPuffs(scaleF, maxPuffs, opts) {
  ...
  // ② 各コブに予算配分した子パフを密に群がらせる（中心寄りに密＝塊感／サイズはコブに比例＝大小のばらつき）
  const puffs = [] // {x,y,z,size,ty}
  for (let i = 0; i < lobes.length; i++) {
    const L = lobes[i]
    const kids = Math.max(5, Math.round(budget * (L.r * L.r) / sumW))
    for (let j = 0; j < kids; j++) {
      const a = Math.random() * 6.2832, el = Math.acos(2 * Math.random() - 1), rr = Math.pow(Math.random(), 0.55) * L.r
      const px = L.x + Math.cos(a) * Math.sin(el) * rr * 0.95
      const pz = L.z + Math.sin(a) * Math.sin(el) * rr * 0.82
      const py = Math.max(0, L.y + Math.cos(el) * rr * (L.flat ? 0.42 : 0.72)) // 底のコブは縦に潰す＝平らな底
      puffs.push({ x: px, y: py, z: pz, size: L.r * (0.52 + Math.random() * 0.34), ty: Math.min(1, Math.max(0, py / H)) })
    }
  }
  // 予算で間引き＝大きいパフ優先（シルエットの骨格を保つ）。基本 kids 合計≈budget なので近景はほぼ無間引き
  if (puffs.length > budget) { puffs.sort((p, q) => q.size - p.size); puffs.length = budget }
  ...
  for (let k = 0; k < N; k++) {
    const p = puffs[k]
    iPos[k * 3] = p.x * scaleF; iPos[k * 3 + 1] = p.y * scaleF; iPos[k * 3 + 2] = p.z * scaleF
    iSize[k] = p.size * scaleF; iY[k] = p.ty; iSeed[k] = Math.random() * 6.2832
  }
...
function unlockIOSAudio() {
  try {
    if (!iosSilent) {
      const rate = 44100, sec = 0.5, n = Math.floor(rate * sec) // 44.1kHz＝iOSのオーディオセッションを低レートに引っ張らない（録画音の劣化を避ける。8kHzから変更2026-06-23）
...
const MB_MOTIFS = [
  ...
  [[5, 0.0], [4, 0.66], [2, 1.5], [3, 2.6], [4, 3.8], [2, 5.1]], // 作り込み（2026-06-27）：上から下りてまた問いかける形
  [[2, 0.0], [4, 0.8], [5, 1.7], [3, 2.9], [1, 4.2], [2, 5.6]],  // ゆるやかな弧
  [[0, 0.0], [1, 0.95], [3, 2.0], [2, 3.2], [0, 4.6]],           // ひかえめな問いかけ（短め）
]
...
function updateMusicBox(dt) {
  ...
  let last = 0
  // 作り込み（2026-06-27）：フレーズの頭に、たまにやわらかなハモり（ペンタの3度下）を1音だけ重ねる＝オルゴールの櫛歯が2枚鳴るような温かみ。控えめ・たまに＝素朴さは壊さない
  if (Math.random() < 0.42) mbNote(Math.max(0, motif[0][0] - 2), now + motif[0][1], vel * 0.42, octBias - (Math.random() < 0.5 ? 1 : 0))
...
// SFXを左右に振る小さなパンナー（蝉を別々の木から鳴らす＝音が中央に固まらず“蝉時雨に囲まれる”自然な広がり・2026-06-27）
function sfxPan(p) { const ctx = listener.context, pan = ctx.createStereoPanner(); pan.pan.value = Math.max(-0.85, Math.min(0.85, p)); pan.connect(getSfxOut()); return pan }
function getNoise() {
...
const FEST_POS = new THREE.Vector2(3124, -186) // 盆踊りの会場＝獅子ヶ谷小学校の校庭(3124,-186)。お囃子はここから聞こえる＝音をたどって校庭の盆踊りへ。buildBonOdoriの設置座標と一致させること（2026-06-24・旧プロト町772,-45から移設）
...
function updateSuika() { // すいか割り＝昼下がりだけ。目隠しの子が棒を振りかぶって振り下ろす／周りの子は指さし応援＋ぴょこ
  const on = (onYato || area === 'yato') && tday > 0.18 && tday < 0.56
  if (suikawari.visible !== on) suikawari.visible = on
  if (!on || !suikaFigs.length) return
  const t = performance.now() * 0.001
  for (const f of suikaFigs) {
    if (f.role === 'swing') { const c = (t * 0.55) % 1, s = c < 0.62 ? -2.6 + (c / 0.62) * 0.5 : -2.1 - Math.sin((c - 0.62) / 0.38 * Math.PI) * 1.5 // 振りかぶり→振り下ろし
      f.armR.rotation.x = s; f.armL.rotation.x = s * 0.55; f.g.rotation.z = (c < 0.62 ? 0 : Math.sin((c - 0.62) / 0.38 * Math.PI) * 0.12) } // 振り下ろしで少し前のめり
    else { const b = Math.sin(t * 3 + f.ph); f.armL.rotation.x = -1.3 + b * 0.25; f.armR.rotation.x = -1.3 - b * 0.25; f.g.position.y = f.baseY + Math.max(0, Math.sin(t * 2 + f.ph)) * 0.05 } // 指さし応援＋ぴょこ
  }
}
...
// G2：遠い呼び声＝夕方の家路、母音だけの「ごはんよー」感（言葉にしない・録音不使用）。
//   近所のお母さんが子を呼ぶ、谷をわたって届く声。近接ゲートに縛られないよう独立配線でマスターへ（遠い＝低いローパス＋うっすらした反響）。
function farCall(t0) { if (!audioStarted) return; try {
  const ctx = listener.context
  const out = ctx.createGain(); out.gain.value = 1
  const lp = ctx.createBiquadFilter(); lp.type = 'lowpass'; lp.frequency.value = 1500; lp.Q.value = 0.4 // 遠い＝高域が届かない
  const dl = ctx.createDelay(); dl.delayTime.value = 0.17; const fb = ctx.createGain(); fb.gain.value = 0.3; const wet = ctx.createGain(); wet.gain.value = 0.5 // 谷をわたる呼び声のうっすらした反響
  dl.connect(fb); fb.connect(dl)
  const pan = lifePan((Math.random() * 2 - 1) * 0.4)
  out.connect(lp); lp.connect(pan); pan.connect(getMaster()); lp.connect(dl); dl.connect(wet); wet.connect(pan)
  const o = ctx.createOscillator(); o.type = 'triangle'
  const vib = ctx.createOscillator(); vib.frequency.value = 5.2; const vibg = ctx.createGain(); vibg.gain.value = 6; vib.connect(vibg); vibg.connect(o.frequency); vib.start(t0); vib.stop(t0 + 2.7) // ゆっくりビブラート＝人のあたたかさ
  const base = 250 + Math.random() * 70
  // 抑揚：「ごはん」相当の短い起伏→「よー」相当へ跳ね上がる呼びかけ→長く尾を引いて落ちる（母音だけ）
  o.frequency.setValueAtTime(base * 0.92, t0)
  o.frequency.linearRampToValueAtTime(base * 1.04, t0 + 0.28)
  o.frequency.linearRampToValueAtTime(base * 0.96, t0 + 0.52)
  o.frequency.linearRampToValueAtTime(base * 1.30, t0 + 0.80)
  o.frequency.linearRampToValueAtTime(base * 1.16, t0 + 1.5)
  o.frequency.linearRampToValueAtTime(base * 0.86, t0 + 2.4)
  // 母音フォルマント（お→あ→お）＝2つのバンドパスで“人の声らしさ”
  ...
  g.gain.linearRampToValueAtTime(pk, t0 + 0.92)       // 「よー」で開く
  g.gain.linearRampToValueAtTime(pk * 0.8, t0 + 1.6)
  g.gain.exponentialRampToValueAtTime(0.0001, t0 + 2.5) // 長い余韻
...
function playBikeBell() { // 自転車のベル「チャリン♪」＝乗った瞬間の合図（小さめ・ユーザー要望2026-06-26）。非整数倍音の澄んだ金属音を2連で
  if (!audioStarted) return
  try { const ctx = listener.context, now = ctx.currentTime
    const ding = (t, f) => { const o = ctx.createOscillator(); o.type = 'triangle'; o.frequency.value = f
      const o2 = ctx.createOscillator(); o2.type = 'sine'; o2.frequency.value = f * 2.76 // 非整数倍音＝鈴の澄んだ響き
      const g2 = ctx.createGain(); g2.gain.value = 0.38; o2.connect(g2)
      const g = ctx.createGain(); g.gain.setValueAtTime(0.0001, t); g.gain.exponentialRampToValueAtTime(0.045, t + 0.004); g.gain.exponentialRampToValueAtTime(0.0001, t + 0.5)
      o.connect(g); g2.connect(g); g.connect(getSfxOut()); o.start(t); o2.start(t); o.stop(t + 0.55); o2.stop(t + 0.55) }
    ding(now, 2340); ding(now + 0.13, 2520) // チャ…リン
  } catch (e) {}
}
function playPedal(vol) { // 自転車をこぐ「カチ…」＝ペダル/チェーンの軽い機械音（ごく小さく・踏むたび）。歩行のplayStepと同様の短いフィルタノイズ
  if (!audioStarted) return
  try { const ctx = listener.context, now = ctx.currentTime
    const src = ctx.createBufferSource(); src.buffer = getNoise(); src.playbackRate.value = 0.9 + Math.random() * 0.2
    const bp = ctx.createBiquadFilter(); bp.type = 'bandpass'; bp.frequency.value = 1050 + Math.random() * 240; bp.Q.value = 1.5
    const g = ctx.createGain(); g.gain.setValueAtTime(0.0001, now); g.gain.exponentialRampToValueAtTime(vol || 0.016, now + 0.004); g.gain.exponentialRampToValueAtTime(0.0001, now + 0.07)
    src.connect(bp); bp.connect(g); g.connect(getSfxOut()); src.start(now); src.stop(now + 0.1)
  } catch (e) {}
}
function playPant(vol) { // 走っている時の弾む息「はっ…はっ」＝軽い吐息（ローパスノイズ・全力で走る時だけまばらに）
  if (!audioStarted) return
  try { const ctx = listener.context, now = ctx.currentTime
    const src = ctx.createBufferSource(); src.buffer = getNoise(); src.playbackRate.value = 0.8 + Math.random() * 0.2
    const lp = ctx.createBiquadFilter(); lp.type = 'lowpass'; lp.frequency.value = 560 + Math.random() * 140; lp.Q.value = 0.7
    const g = ctx.createGain(); g.gain.setValueAtTime(0.0001, now); g.gain.exponentialRampToValueAtTime(vol || 0.02, now + 0.03); g.gain.exponentialRampToValueAtTime(0.0001, now + 0.2)
    src.connect(lp); lp.connect(g); g.connect(getSfxOut()); src.start(now); src.stop(now + 0.26)
  } catch (e) {}
}
let catPurr = null
...
// M2（2026-06-27）：夜の音層を厚く＝遠いカエルの合唱(遠近2層)＋たまにふくろう。自前合成・まばら（眠れる静けさを壊さない）
let frogCd = 2, owlCd = 30
function playFrog(near) { // ゲコ…ゲコ（低い蛙）。near=手前は少し大きく明るく、遠=getDistantOutでこもって小さく＝遠近の2層
  try { const ctx = listener.context, out = near ? getSfxOut() : getDistantOut(), n = 2 + Math.floor(Math.random() * 3), f0 = (near ? 215 : 165) + Math.random() * 40
    for (let i = 0; i < n; i++) { const t0 = ctx.currentTime + 0.05 + i * (0.2 + Math.random() * 0.14)
      const osc = ctx.createOscillator(); osc.type = 'sawtooth'; osc.frequency.setValueAtTime(f0 * 1.1, t0); osc.frequency.exponentialRampToValueAtTime(f0 * 0.82, t0 + 0.1)
      const bp = ctx.createBiquadFilter(); bp.type = 'bandpass'; bp.frequency.value = f0 * 2.2; bp.Q.value = 5
      const g = ctx.createGain(), v = near ? 0.05 : 0.03
      g.gain.setValueAtTime(0.0001, t0); g.gain.linearRampToValueAtTime(v, t0 + 0.02); g.gain.setValueAtTime(v, t0 + 0.06); g.gain.exponentialRampToValueAtTime(0.0001, t0 + 0.13)
      osc.connect(bp); bp.connect(g); g.connect(out); osc.start(t0); osc.stop(t0 + 0.16) } } catch (e) {}
}
function playOwl() { // ホー、ホー（ふくろう）。やわらかいサイン2音＝こもらせて遠くに
  try { const ctx = listener.context, out = getDistantOut()
    for (let i = 0; i < 2; i++) { const t0 = ctx.currentTime + 0.1 + i * 0.72, f = 380 + Math.random() * 26
      const osc = ctx.createOscillator(); osc.type = 'sine'; osc.frequency.setValueAtTime(f, t0); osc.frequency.linearRampToValueAtTime(f * 0.93, t0 + 0.4)
      const g = ctx.createGain()
      g.gain.setValueAtTime(0.0001, t0); g.gain.linearRampToValueAtTime(0.04, t0 + 0.08); g.gain.setValueAtTime(0.034, t0 + 0.3); g.gain.exponentialRampToValueAtTime(0.0001, t0 + 0.5)
      osc.connect(g); g.connect(out); osc.start(t0); osc.stop(t0 + 0.55) } } catch (e) {}
}
function maybeNightCreatures(dt) {
  if (!audioStarted) return
...
let dayCicadaCd = 6
function playMinmin() { // ミンミンゼミ「ミ〜ンミンミンミン・ミ〜」＝“濾したノイズの共鳴”（蝉は純音でなくノイズの羽音／オシレータの電子音=ピロロロを廃し自然な羽音に・ユーザー要望2026-06-27）
  try { const ctx = listener.context, out = sfxPan((Math.random() * 2 - 1) * 0.6), t0 = ctx.currentTime + 0.05, dur = 2.2 + Math.random() * 1.3, base = 2650 + Math.random() * 380
    const n = ctx.createBufferSource(); n.buffer = getNoise(); n.loop = true // 羽音の芯＝白色ノイズ
    const bp1 = ctx.createBiquadFilter(); bp1.type = 'bandpass'; bp1.frequency.value = base; bp1.Q.value = 16 // 高Qで“濁った音程感”＝ジーンと鳴る（純音にしない）
    const bp2 = ctx.createBiquadFilter(); bp2.type = 'bandpass'; bp2.frequency.value = base * 2.0; bp2.Q.value = 13 // 倍音の共鳴＝蝉の硬い響き
    const peak = ctx.createGain(); peak.gain.value = 1.0
    const trem = ctx.createGain(); trem.gain.value = 0.6 // トレモロ＝「ミンミンミン」の刻み（イントロ遅→本体速→アウトロ遅）
    const lfo = ctx.createOscillator(); lfo.type = 'sine'; lfo.frequency.setValueAtTime(2.2, t0); lfo.frequency.linearRampToValueAtTime(8.5, t0 + 0.55); lfo.frequency.setValueAtTime(8.5, t0 + dur - 0.5); lfo.frequency.linearRampToValueAtTime(3, t0 + dur)
    const lfoG = ctx.createGain(); lfoG.gain.value = 0.38; lfo.connect(lfoG); lfoG.connect(trem.gain)
    const master = ctx.createGain()
    n.connect(bp1); n.connect(bp2); bp1.connect(peak); bp2.connect(peak); peak.connect(trem); trem.connect(master); master.connect(out)
    master.gain.setValueAtTime(0.0001, t0); master.gain.exponentialRampToValueAtTime(0.03, t0 + 0.4); master.gain.setValueAtTime(0.03, t0 + dur - 0.6); master.gain.exponentialRampToValueAtTime(0.0001, t0 + dur)
    n.start(t0); n.stop(t0 + dur + 0.15); lfo.start(t0); lfo.stop(t0 + dur + 0.15)
  } catch (e) {}
}
function playTsukutsuku() { // ツクツクボウシ「ツクツクツク…(だんだん速く)…オーシ↘」＝濾したノイズのラスプ（電子的な矩形波のピッ音=ピロロロを廃し、擦れた羽音に・ユーザー要望2026-06-27）
  try { const ctx = listener.context, out = sfxPan((Math.random() * 2 - 1) * 0.6), t0 = ctx.currentTime + 0.05, noise = getNoise()
    let tt = t0, rate = 0.2; const n = 7 + Math.floor(Math.random() * 4)
    for (let i = 0; i < n; i++) { // 「ツク」＝短い帯域ノイズのバースト（擦れたチッ／純音にしない）
      const ns = ctx.createBufferSource(); ns.buffer = noise; ns.loop = true
      const bp = ctx.createBiquadFilter(); bp.type = 'bandpass'; bp.frequency.value = 3100 + Math.random() * 360; bp.Q.value = 5.5
      const g = ctx.createGain(); g.gain.setValueAtTime(0.0001, tt); g.gain.linearRampToValueAtTime(0.019, tt + 0.012); g.gain.exponentialRampToValueAtTime(0.0001, tt + 0.085)
      ns.connect(bp); bp.connect(g); g.connect(out); ns.start(tt); ns.stop(tt + 0.1); tt += rate; rate *= 0.93 } // だんだん速く
    const bt = tt + 0.1 // 「オーシ↘」＝下降する帯域ノイズの伸ばし（擦れたまま音程が下がる）
    const ns2 = ctx.createBufferSource(); ns2.buffer = noise; ns2.loop = true
    const bp2 = ctx.createBiquadFilter(); bp2.type = 'bandpass'; bp2.Q.value = 6; bp2.frequency.setValueAtTime(2950, bt); bp2.frequency.exponentialRampToValueAtTime(1650, bt + 0.5)
    const g2 = ctx.createGain(); g2.gain.setValueAtTime(0.0001, bt); g2.gain.linearRampToValueAtTime(0.026, bt + 0.07); g2.gain.setValueAtTime(0.022, bt + 0.34); g2.gain.exponentialRampToValueAtTime(0.0001, bt + 0.58)
    ns2.connect(bp2); bp2.connect(g2); g2.connect(out); ns2.start(bt); ns2.stop(bt + 0.62)
  } catch (e) {}
}
function playNiinii() { // ニイニイゼミ「チー…ジー…」＝細く高い持続音(約4.5kHz)に細かなチリつき。控えめ・長め＝薄い層（J3・ユーザー要望2026-06-27）
  try { const ctx = listener.context, out = sfxPan((Math.random() * 2 - 1) * 0.6), t0 = ctx.currentTime + 0.05, dur = 3.4 + Math.random() * 2.6, base = 4350 + Math.random() * 420
    ...
    master.gain.setValueAtTime(0.0001, t0); master.gain.exponentialRampToValueAtTime(0.016, t0 + 0.6); master.gain.setValueAtTime(0.016, t0 + dur - 0.8); master.gain.exponentialRampToValueAtTime(0.0001, t0 + dur) // 薄い層なので控えめ（J3→2026-06-27バランス調整）
...
function maybeDayCicadas(dt) { // 昼だけ、ミンミン/ツクツク/ニイニイをまばらに（アブラゼミの基底に重ねて“いろんな蝉”に）
  if (!audioStarted) return
  if (cicHushT > 2.5) return // ★near-silence中（ふっと鳴き止む瞬間）は重ねる蝉を完全カット＝近くの蝉が黙り遠くの基底だけ残る＝静寂が際立つ
...
function openDiary() {
  ...
  if (todayFlags.rodeSlide) body.push('長い ローラーすべり台を すべった。おしりが くすぐったくて、風が ぴゅうって 鳴った。')
...
let activePond = null // いま釣れる最寄りの池（はらっぱの池 or 谷戸の池）。update で毎フレーム決める
let castPond = null   // 糸を垂れている最中の池（途中で少し動いても浮きの高さがズレないよう保持）
let floatWy = 0       // 浮きを落とした水面の高さ（谷戸の池は地形で水面が変わるので保持）
// 谷戸の池は多角形（SG.waters）。中心+半径だと細長い二ツ池で岸が範囲外になるので、岸（多角形の辺）からの最短距離で「池のそば」を判定する。
function segDist2(px, pz, ax, az, bx, bz) { const dx = bx - ax, dz = bz - az, l2 = dx * dx + dz * dz; let t = l2 ? ((px - ax) * dx + (pz - az) * dz) / l2 : 0; t = t < 0 ? 0 : t > 1 ? 1 : t; return Math.hypot(px - (ax + t * dx), pz - (az + t * dz)) }
function nearestPondForFishing(x, z) {
  let best = null, bestD = 1e9
  for (const P of YATO_PONDS) {
    if (Math.hypot(x - P.cx, z - P.cz) > P.br + 12) continue // 粗いふるい（外接円＋余裕）
    if (pip(x, z, P.p)) return P // 水の上に立っていれば即これ
    const p = P.p; let ed = 1e9
    for (let i = 0, j = p.length - 1; i < p.length; j = i++) ed = Math.min(ed, segDist2(x, z, p[j][0], p[j][1], p[i][0], p[i][1]))
    if (ed < bestD) { bestD = ed; best = P }
  }
  return bestD <= 7 ? best : null // 岸から7m以内なら「釣れる」
}
// C⑫：中央下スロット(left:50%/bottom:15%)を共有する文脈ボタン(すわる/はなす/つかまえる/つる/町へ)は、
//   複数が同時に出ると ぴったり重なって読めなくなる(UI/UX指摘#3＝#go/#act衝突)。表示中のものだけを
//   優先順で縦に積む(translateYで持ち上げ＝bottomは触らないので横画面メディアクエリ/safe-areaを保つ)。
const ctxStack = [actBtn, catchEl, fishEl, npcEl, goEl] // 下→上の優先＝その場の操作を親指側(下)に、移動の「町へ」を上に
function layoutCtxButtons() {
  let slot = 0
  for (const el of ctxStack) {
    if (!el) continue
    el.classList.remove('ctx-up1', 'ctx-up2', 'ctx-up3', 'ctx-up4')
    if (el.style.display !== 'block') continue // 表示中(inline display:block)だけ積む
    if (slot > 0) el.classList.add('ctx-up' + slot) // 先頭は基準位置のまま・2つ目以降を持ち上げ
    slot++
  }
}
...
function castLine() {
  const P = activePond; if (!P) return // 池のそばでなければ投げない
  castPond = P
  let dx = P.cx - boy.position.x, dz = P.cz - boy.position.z; const dl = Math.hypot(dx, dz) || 1; dx /= dl; dz /= dl // 池の中心へ向けて
  let fx = boy.position.x + dx * 2.5, fz = boy.position.z + dz * 2.5, wy
  if (P.yato) { for (let s = 1.5; s <= 18; s += 1.0) { const tx = boy.position.x + dx * s, tz = boy.position.z + dz * s; if (pip(tx, tz, P.p)) { fx = tx; fz = tz; break } } wy = heightAtYato(fx, fz) + 0.25 } // 水に入る最初の点へ浮きを落とす（岸が遠い大きな池でもちゃんと水面に）
  else wy = P.y
  floatWy = wy
  floatMesh.position.set(fx, wy, fz)
  floatMesh.visible = true
  playPlop(); spawnRipple(fx, fz) // 浮きが水に落ちる「ぽちゃん」＋波紋
...
window.__build = '20260626-slideui' // ビルド識別（HTMLのみ変更時もバンドル名を変えて自動更新を効かせるため）
...
function doJump() { if (jumpY <= 0.02 && mode === 'walk' && !flying && !floatMode) { jumpV = riding ? 6.2 : 7.0; airborne = true; playJump(); todayFlags.jumped = true } } // 接地時だけ跳ねる＋ジャンプ音（自転車中も走りながらぴょん＝少し低めに跳ねる・ユーザー要望2026-06-26。浮遊中は跳ねない）
...
function rideSlide() {
  ...
  sliding = { s: 0.3, v: 4.5, pov: 'first', lean: 0, lookYaw: 0, lookPitch: 0 } // 弧長(m)と速さ(m/s)・既定は主観視点・lean=カーブの傾き・look*=見回しのずれ（初速は最低速4.5に合わせる）
  boy.scale.setScalar(BOY_SCALE); landSquash = 0; airborne = false
...
function update(dt) {
  ...
  if (audioStarted) {
    ...
    // 蝉しぐれ：寄せては返すように音量がうねる（一様でない＝本物の夏の気配）。協議2026-06-27＝うねりを深く・ループに聞こえないよう互いに素な周期を重ねる
    // M2：蝉の息継ぎ＝数分に一度、6秒かけて蝉がすっと静まる“間”（谷を0.42→0.22に深く・頻度を上げる）
    cicBreathCd -= dt; if (cicBreathCd <= 0 && cicBreathT <= 0) { cicBreathCd = 70 + Math.random() * 70; cicBreathT = 6 }
  ...
  for (const c of yatoBugs) {
    ...
    if (c.kind === 'tombo') {
      ...
      const flap = 0.62 + Math.sin(tsec * 12 + c.ph) * 0.5; c.wl.rotation.y = flap; c.wr.rotation.y = -flap // 常に半開き＝平らな板にならずヒラヒラ
  ...
  if (mode === 'walk') {
    ...
    if (moving && !riding && !floatMode && sw * lastStepS < 0 && jumpY <= 0.02 && !airborne) { // ジャンプ中(空中)/自転車/浮遊は足音を出さない
      if (inCreek) { playPlop(); spawnRipple(boy.position.x, boy.position.z); spawnRipple(boy.position.x + (Math.random() - 0.5) * 0.8, boy.position.z + (Math.random() - 0.5) * 0.8) }
        ...
        playStep(0.042 + run * 0.1, surf); if (run > 0.4 && surf !== 'pave') spawnDust(boy.position.x, boy.position.y + 0.05, boy.position.z) // 走るほど足音を強く＝自然な“走ってる音”（ユーザー要望2026-06-26）。砂ぼこりは土/草で
    ...
    else if (moving && speedNow > 0.8 && !boy.userData._high && !fpv) { // 屋上(高所)/主観視点では自動追従を切り、指で自由に視点を回せる＝街を一望
    ...
    sliding.v = Math.min(13.0, Math.max(4.5, sliding.v + (38 * slope - 0.26 * sliding.v) * dt)) // 重力で加速（上限13m/s＝全体的にもう少し速く）。蛇行で長く緩くなった分、加速38・抗力0.26・最低4.5に上げて末尾の失速を解消（ユーザー要望2026-06-26）
...
window.__proto3d = {
  ...
  _roadGaps() { const out = []
    for (const [x0, z0, x1, z1] of roadSegs) { const L = Math.hypot(x1 - x0, z1 - z0) || 1, dx = (x1 - x0) / L, dz = (z1 - z0) / L
      for (const [ex, ez, ddx, ddz] of [[x0, z0, -dx, -dz], [x1, z1, dx, dz]]) {
        const cont = onYatoRoad(ex + ddx * 2.5, ez + ddz * 2.5) || onYatoRoad(ex + ddx * 4, ez + ddz * 4) // 外側に道が続くか
        const cross = onYatoRoad(ex - ddz * 2.5, ez + ddx * 2.5) || onYatoRoad(ex + ddz * 2.5, ez - ddx * 2.5) // 横(交差)に道があるか
        if (!cont && !cross) out.push({ x: Math.round(ex), z: Math.round(ez), seg: [Math.round(x0), Math.round(z0), Math.round(x1), Math.round(z1)] }) } }
    return out }, // 検証用：手描き道の端点で外側にも交差にも道が無い＝途切れ/行き止まり候補（一部は正当な行き止まり）
  _buildingRoadAudit(thresh) { const t = thresh || 0.03, bad = []
    for (const [cx, cz, w, d, ang] of builtBuildings) { const co = Math.cos(ang), si = Math.sin(ang), hw = w / 2, hd = d / 2
      const nx = Math.max(2, Math.round(w / 2)), nz = Math.max(2, Math.round(d / 2)); let on = 0, tot = 0
      ...
      const frac = on / tot; if (frac > t) bad.push({ x: Math.round(cx), z: Math.round(cz), w: Math.round(w), d: Math.round(d), roadFrac: +frac.toFixed(2) }) }
    return bad.sort((a, b) => b.roadFrac - a.roadFrac) }, // 検証用：建った建物のうちフットプリントが道路の舗装に乗っているもの（占有率つき）
  _worldAudit() { initFishers()
    const desc = (x, z) => npcInWater(x, z) ? '水中' : onYatoRoadCore(x, z) ? '道路上' : deepInBuilding(x, z) ? '建物内' : 'OK' // 建物は「深く埋まる」だけ＝壁際(店先/路傍)の自然な配置は除外
    const bad = [], P = (o) => o && o.position
    // walker=道を歩く者(通行人/猫)は道路上が正常＝水/建物だけ見る。stationary=立ち止まる物は道路上もNG
    const chk = (type, x, z, walker) => { if (x == null) return; const d = desc(x, z); if (d === 'OK') return; if (walker && d === '道路上') return; if (type === 'fisher' && d === '水中') return; bad.push({ type, x: Math.round(x), z: Math.round(z), at: d }) }
    for (const [n, o] of [['villager', typeof villager !== 'undefined' && villager], ['townLady', typeof townLady !== 'undefined' && townLady], ['townKid', typeof townKid !== 'undefined' && townKid], ['farmer', typeof farmer !== 'undefined' && farmer]]) if (P(o)) chk(n, o.position.x, o.position.z)
    for (const p of pedestrians) if (P(p)) chk('ped', p.position.x, p.position.z, true) // 通行人は歩く＝道路上OK
    if (typeof shopkeepers !== 'undefined') for (const p of shopkeepers) if (P(p)) chk('shop', p.position.x, p.position.z)
    if (typeof festFigs !== 'undefined') for (const f of festFigs) if (P(f.g)) chk('fest', f.g.position.x, f.g.position.z)
    if (typeof taisoFigs !== 'undefined') for (const f of taisoFigs) if (P(f.g)) chk('taiso', f.g.position.x, f.g.position.z)
    if (typeof suikaFigs !== 'undefined') for (const f of suikaFigs) if (P(f.g)) chk('suika', f.g.position.x, f.g.position.z)
    if (typeof kidsCatch !== 'undefined' && kidsCatch) for (const k of [kidsCatch.a, kidsCatch.b, kidsCatch.w]) if (P(k)) chk('catch', k.position.x, k.position.z)
    if (typeof cats !== 'undefined') for (const c of cats) if (P(c)) chk('cat', c.position.x, c.position.z, true) // 猫は歩く＝道路上OK
    // 犬は家につながれた番犬＝家のそば(建物内判定)で正常なので監査対象外
    for (const f of fishers) chk('fisher', f.g.position.x, f.g.position.z) // 釣り人本体（浮きは水中で正常なので除外）
    for (const g of runGroups) chk('run', g.cx, g.cz)
    for (const c of chatPairs) chk('chat', c.cx, c.cz)
    if (typeof auditProps !== 'undefined') for (const pr of auditProps) chk(pr.kind, pr.x, pr.z) // 電柱/自販機/看板など小物
    return bad }, // 検証用：全NPC＋小物のうち 水中/道路上/建物内 に立つ"違反"だけを返す（空=漏れなし）
  ...
  get _fish() { return { state: fishState, near: !!activePond, pond: activePond, count: fish.count, ponds: YATO_PONDS.length } }, // 検証用：釣りの状態
  _castLine() { castLine() }, _reel() { reel() }, // 検証用：釣り糸を投げる/引く
  YATO_PONDS, // 検証用：谷戸の釣り場一覧
  _yatoBugs() { return catchables.filter((c) => !c.done && c.obj.position.x > 2000).map((c) => ({ x: c.obj.position.x, z: c.obj.position.z, kind: c.kind })) }, // 検証用：谷戸の虫の位置（虫採りが谷戸で成立するか）
  _npf(x, z) { const p = nearestPondForFishing(x, z); return p ? { cx: Math.round(p.cx), cz: Math.round(p.cz), br: Math.round(p.br) } : null }, // 検証用：その点で釣れる池（update非依存で直接判定）
  ...
  _minmin() { startAudio(); try { listener.context.resume() } catch (e) {} playMinmin(); return { state: listener.context.state } }, // 検証用：ミンミンゼミ
