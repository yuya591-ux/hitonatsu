...
const bonOdori = new THREE.Group(); bonOdori.visible = false; scene.add(bonOdori) // 盆踊り会場（櫓＋提灯）＝小学校の校庭。開催日だけ姿を見せる
{
  ...
  // ── 【裏山の谷を下る3本目の道／ユーザー要望・森を迂回する形に変更】二股の“間”から谷を下る。西側に森を並べ、道はその森の“東側”を回り込みながら下って北の新しい土地へ ──
  makeRoadRibbon(T.x - 78, T.z + 88, T.x - 70, T.z + 120, 4, false, true)  // (922,88)→(930,120) 二股の間から、森の東へ回り込んで谷へ下り始める
  makeRoadRibbon(T.x - 70, T.z + 120, T.x - 60, T.z + 155, 4, false, true) // (930,120)→(940,155) 森の東側を下る
  makeRoadRibbon(T.x - 60, T.z + 155, T.x - 59, T.z + 195, 4, false, true) // (940,155)→(941,195) 山を抜けて新しい土地へ
  makeRoadRibbon(T.x - 59, T.z + 195, T.x - 62, T.z + 230, 4, false, true) // (941,195)→(938,230) さらに北へ
  makeSignpost(T.x - 73, T.z + 92, Math.PI, '谷の道 ↓') // 二股の間＝谷を下る道の道しるべ
  // 西側の森（主人公がいた辺り→北へ、谷の道の左/西に並べる。道はこの森の東を迂回する・ユーザー要望）
  for (const [tx, tz, ts] of [
    [916, 132, 1.2], [902, 128, 1.4], [888, 134, 1.1], [872, 130, 1.3],
    [914, 152, 1.3], [900, 150, 1.1], [884, 154, 1.4], [870, 150, 1.2],
    [916, 174, 1.1], [902, 170, 1.3], [886, 176, 1.2], [870, 172, 1.4],
    [914, 196, 1.3], [898, 198, 1.1], [882, 194, 1.3], [868, 198, 1.2],
    [908, 216, 1.2], [892, 214, 1.4], [876, 218, 1.1],
  ]) makeTree(tx, tz, ts)
  // ── ブランコ（乗ってブランコ視点であそぶ）──
  {
