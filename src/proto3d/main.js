...
const SUNRISE_HILL = { x: TOWN.x - 82, z: TOWN.z - 24, h: 7.5, w: 12, d: 10 } // サンライズ(作者の家)の建つ丘＝裏山とは別の高台（鶴見の山坂を再現）
...
const townNightLights = [] // 夜に灯る街のあかり（窓・街灯・自販機）。nfで点灯＝夜のエモさ
{
  ...
  function makeMansion(cx, cz) {
    ...
    const base = new THREE.Mesh(new THREE.BoxGeometry(W, baseH, D), toonMap(0x8c8c88, plasterTex)); base.position.y = baseH / 2; g.add(base)
    for (let u = 0; u < units; u++) { const op = new THREE.Mesh(new THREE.PlaneGeometry(1.7, 1.9), toon(0x26262a)); op.position.set(-W / 2 + 1.3 + u * 2.6, baseH * 0.55, D / 2 + 0.03); g.add(op) }
    // 居室7階（グレー）
    ...
    const stair = new THREE.Mesh(new THREE.BoxGeometry(1.5, baseH + floors * FH, 1.8), toonMap(0xacaca6, plasterTex)); stair.position.set(W / 2 + 0.75, (baseH + floors * FH) / 2, D / 2 - 0.9); g.add(stair) // 外階段塔
    const ent = new THREE.Mesh(new THREE.BoxGeometry(2.6, 2.4, 0.5), toon(0x5a6068)); ent.position.set(-W * 0.28, baseH * 0.5 + 0.1, D / 2 + 0.22); g.add(ent) // 道路側のエントランス
    const entdoor = new THREE.Mesh(new THREE.PlaneGeometry(1.6, 2.0), toon(0x2e3a40)); entdoor.position.set(-W * 0.28, baseH * 0.5, D / 2 + 0.48); g.add(entdoor)
    g.traverse((o) => { if (o.isMesh) o.castShadow = true })
