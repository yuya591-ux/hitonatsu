...
// 長い坂道の高さ。坂は“南(小さいz)ほど高い”＝北(下/しんみせ)→南へ登る→途中の平らな踊り場(ビスコ)→マンション(約7割)→頂上(南)。
function slopeHeight(z) {
  const zb = TOWN.z + 12, zl1 = TOWN.z - 2, zl0 = TOWN.z - 14, ztop = TOWN.z - 36, hL = 7, hT = 22
...
const townNightLights = [] // 夜に灯る街のあかり（窓・街灯・自販機）。nfで点灯＝夜のエモさ
{
  ...
  makeRoadRibbon(T.x - 76, T.z + 18, T.x - 76, T.z - 44, 9) // 長く大きい坂道（尾根の道）：北(下/しんみせ) → 南(上/マンション・頂上)。マンションは坂の約7割地点の西脇
  // 上り(南)＝頂上→北寺尾の町方面／下り(北)＝ビスコ(踊り場)→しんみせ(一番下)→本通り
  makeRoadRibbon(T.x - 76, T.z + 14, T.x - 4, T.z + 6, 6) // 坂下(北)を町の本通りへ接続
  ...
  makeGameShop(T.x - 86, T.z - 8, Math.PI / 2) // 坂の途中の“踊り場”の左手(西)＝ビスコ（店先＝東の車道側／登る(南へ)と左手に見える）
  ...
  makeDagashi(T.x - 84, T.z + 10, Math.PI / 2) // 坂の一番下(北)＝しんみせ（新店・道の西脇・店先＝東の車道側）
