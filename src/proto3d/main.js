...
// ── サンライズ北寺尾(獅子ヶ谷・実物)の屋上：7階建てRC。NW端の開放テラス(部屋なし)から、よく景色を見渡した実体験を再現。屋上を歩けて、前の階段で登り降りできる ──
...
const SUN_TZONE = (x, z) => ((x - 2981) * 1.2 - (-z - 20.7) * 15.9) < 0 // NW端＝部屋のない開放テラス領域（一段低い）
...
const SUN_ROOF = (() => { let gmax = -1e9; for (const [x, z] of SUN_POLY) { const e = heightAtYato(x, z); if (e > gmax) gmax = e } const F = 3, base = gmax + 1.5; return { base, t6: base + 6 * F, top: base + 7 * F } })() // 本体陸屋根top・開放テラス床t6（makeのbase/F/階数と一致）
function sunriseYatoClimbY(x, z) { if (!pointInSunPoly(x, z)) return null; return SUN_TZONE(x, z) ? SUN_ROOF.t6 : SUN_ROOF.top } // 屋上の歩行面：テラス=t6/本体=top。輪郭の外はnull＝縁で落下防止が効く（壁コライダーで地上からは入れないので地上は誤判定しない）
function climbYAt(x, z) { return x < 2200 ? sunriseClimbY(x, z) : sunriseYatoClimbY(x, z) } // 旧町のマンション屋上 と 獅子ヶ谷の実サンライズ屋上 を一本化
...
function buildShishigaya() {
  ...
  const skipZones = [[3002, -148, 28], [2968, -104, 13], [3012, -56, 24], ...NAMED.map((n) => [n[0], n[1], n[4]])]
  ...
  { const poly = SUNRISE_POLY
    ...
    for (let k = 0; k < poly.length; k++) { const a = poly[k], b = poly[(k + 1) % poly.length], openEdge = tzone(a[0], a[1]) && tzone(b[0], b[1]) // テラス辺は6階まで＋手すり、他は7階まで
      ...
      if (openEdge) { const r0 = [a[0], t6, a[1]], r1 = [b[0], t6, b[1]], r2 = [b[0], t6 + 1.2, b[1]], r3 = [a[0], t6 + 1.2, a[1]]; pushTri(conc, r0, r1, r2); pushTri(conc, r0, r2, r3) } } // テラスの欄干
  ...
  // ───── サンライズの屋上へ登る外階段＋本体屋上のパラペット：屋上を歩いて、NW端の開放テラスから景色を見渡せるように（ユーザーが昔よくやっていた）。高さは SUN_ROOF/SUN_POLY と一致 ─────
  { const grp = new THREE.Group(); grp.name = 'sunriseRoofAccess'; scene.add(grp)
    const conc = toon(0xbab5a8), rail = toon(0x9a9a90)
    const RM = (geo, mat, x, y, z, ry, rx) => { const m = new THREE.Mesh(geo, mat); m.position.set(x, y, z); if (ry || rx) { m.rotation.order = 'YXZ'; if (ry) m.rotation.y = ry; if (rx) m.rotation.x = rx } m.castShadow = true; m.receiveShadow = true; grp.add(m); return m }
    // 本体屋上のパラペット（外周のへり）＝腰高で落ちない安心感。テラスの縁は既存の欄干があるので主屋根の辺だけに回す
    for (let k = 0; k < SUN_POLY.length; k++) { const a = SUN_POLY[k], b = SUN_POLY[(k + 1) % SUN_POLY.length], len = Math.hypot(b[0] - a[0], b[1] - a[1]); if (len < 1) continue
      if (SUN_TZONE(a[0], a[1]) && SUN_TZONE(b[0], b[1])) continue // テラス縁は既存の欄干にまかせる
      const cx = (a[0] + b[0]) / 2, cz = (a[1] + b[1]) / 2, ang = Math.atan2(b[1] - a[1], b[0] - a[0])
      RM(new THREE.BoxGeometry(len, 1.1, 0.24), conc, cx, SUN_ROOF.top + 0.55, cz, ang) // 立ち上がり
      RM(new THREE.BoxGeometry(len, 0.1, 0.34), rail, cx, SUN_ROOF.top + 1.12, cz, ang) } // 笠木（手すり天端）
    // 外階段：街側(SE)の面に沿う「折り返し階段」。地上→踊り場→…→屋上。緩い勾配で、はしごでなく団地の外階段らしく
    { const A = SUN_POLY[0], B = SUN_POLY[1], L0 = Math.hypot(B[0] - A[0], B[1] - A[1])
      const ux = (B[0] - A[0]) / L0, uz = (B[1] - A[1]) / L0, nx = -uz, nz = ux // û=面に沿う向き／n=外向き法線（多角形の外＝街側）
      const head = Math.atan2(ux, uz), s0 = 5, Lf = 6.5, off1 = 2.6, off2 = 4.8, NF = 6, wT = 1.9 // 2レーン(off1=壁側/off2=外側)で折り返す。NF段で屋上へ
      const P = (s, off) => [A[0] + ux * s + nx * off, A[1] + uz * s + nz * off] // 面上位置s・外向きoff → 座標
      const foot = P(s0, off1), y0 = heightAtYato(foot[0], foot[1]), yTop = SUN_ROOF.top, rise = (yTop - y0) / NF
      for (let k = 0; k < NF; k++) { const dir = (k % 2 === 0) ? 1 : -1, off = (k % 2 === 0) ? off1 : off2, yb = y0 + k * rise
        const nT = 5, dRun = Lf / nT, dY = rise / nT, startS = (dir > 0) ? s0 : s0 + Lf
        for (let i = 0; i < nT; i++) { const s = startS + dir * (i + 0.5) * dRun, p = P(s, off), sy = yb + (i + 0.5) * dY
          RM(new THREE.BoxGeometry(wT, 0.16, dRun + 0.3), conc, p[0], sy + 0.08, p[1], head) // 踏面
          const r = P(s - dir * dRun / 2, off); RM(new THREE.BoxGeometry(wT, dY + 0.06, 0.12), conc, r[0], sy + dY / 2, r[1], head) } // 蹴上げ
        const landS = (dir > 0) ? s0 + Lf : s0, lp = P(landS, (off1 + off2) / 2)
        RM(new THREE.BoxGeometry(off2 - off1 + wT, 0.2, wT + 0.4), conc, lp[0], yb + rise, lp[1], head) // 踊り場（レーン間を渡す＝折り返し）
        for (const oo of [off - wT / 2 - 0.05, off + wT / 2 + 0.05]) { const pp = P(startS + dir * Lf, oo); RM(new THREE.CylinderGeometry(0.05, 0.05, 1.0, 5), rail, pp[0], yb + rise + 0.5, pp[1]) } } // 折り返しの手すり支柱
      const bp = P(s0, off1 / 2); RM(new THREE.BoxGeometry(off1 + wT + 1.2, 0.22, wT + 0.4), conc, bp[0], yTop, bp[1], head) // 最上段→屋上の縁へ渡る橋
      addBox(foot[0], foot[1], 3.4, 3.0, head, 0.3) } // 階段の足元にだけ当たり判定（地上ですり抜けない）
  }
  ...
  { const grp = new THREE.Group(); grp.name = 'sunriseSurround'; scene.add(grp)
    ...
    const sx = 2968, sz = -104, sg = heightAtYato(sx, sz), shop = new THREE.Group(); shop.position.set(sx, sg, sz); shop.rotation.y = Math.atan2(3008 - sx, -8 - sz); grp.add(shop) // (2) ゲームショップ「ビスコ」：サンライズ北西(ユーザー地図のピン・グラウンドと重ならないよう離す)。正面を上手(サンライズ側)へ
  ...
  { const grp = new THREE.Group(); grp.name = 'landmarks2'; scene.add(grp)
    ...
    const buildSchoolDetailed = (cx, cz, name) => { // 獅子ヶ谷小学校＝入口に広場→奥に階段を上ってグラウンド（実際の形）
      ...
      const px = cx - up * 32, pg = gmin4(px, cz, 26, 30); grp.add(mk(new THREE.BoxGeometry(26, 0.4, 30), toon(0xc2bdb2), px, pg + 0.12, cz, 0, true)) // 入口の広場(舗装)
      const gxx = cx + up * 36; ground(gxx, cz, 50, 42, 0xccb78a) // 奥の高い造成グラウンド(土)
      const lo = pg + 0.3, hi = gmax4(gxx, cz, 50, 42) + 0.3, steps = Math.max(3, Math.round((hi - lo) / 0.7)) // 広場→グラウンドの階段
      for (let s = 0; s < steps; s++) grp.add(mk(new THREE.BoxGeometry(1.6, 0.34, 11), toon(0xcac4b8), cx + up * (8 + s * 1.5), lo + (s + 0.5) * (hi - lo) / steps, cz, 0, true))
...
boy.position.set(912, heightAt(912, -73), -73); boy.rotation.y = Math.PI / 2 // ゲーム開始位置＝サンライズ(マンション)の入口を出た私道の上・東(道のほう)向き。マンションを坂上へ移設(z-23・2026-06-19)
outlineObj(boy, 0.03)
...
window.__build = '20260622-sunrise-roof' // ビルド識別（HTMLのみ変更時もバンドル名を変えて自動更新を効かせるため）
...
// サンライズの屋上へ（前の階段を上がる演出＝カメラがすっと昇る）。屋上は歩けて、NW端の開放テラスから景色を見渡せる
function sunGoRoof() {
  if (mode !== 'walk') return
  endPuni(); vel.set(0, 0, 0); moving = false
  const tx = 3018, tz = 4 // 本体屋上の出口付近（EV塔屋の脇）に立つ
  boy.position.set(tx, SUN_ROOF.top, tz)
  facing = Math.atan2(2989 - tx, -23 - tz); boy.rotation.y = facing // 開放テラス(眺望のNW)の方を向く
  boy.userData._cy = SUN_ROOF.top; boy.userData._cx = tx; boy.userData._cz = tz // 落下防止の基準を屋上に
  camCtl.yaw = facing + Math.PI; camManualTimer = 0 // カメラを背後へ＝正面に町と遠景
  todayFlags.climbedRoof = true
}
// 屋上から地上（前の入口）へおりる
function sunLeaveRoof() {
  if (mode !== 'walk') return
  endPuni(); vel.set(0, 0, 0); moving = false
  const gx = 3012, gz = 25 // 入口の前（建物の外・坂上=南側の地上。屋根の真下でない＝屋上に吸い戻されない）
  boy.position.set(gx, heightAt(gx, gz), gz); facing = Math.atan2(3018 - gx, 4 - gz); boy.rotation.y = facing
  boy.userData._cy = null; boy.userData._cx = gx; boy.userData._cz = gz // _cyをnull＝「屋上から落ちた」と誤判定して引き戻されないように
}
...
function update(dt) {
  ...
  if (mode === 'walk') {
    ...
    let climbY = climbYAt(boy.position.x, boy.position.z)
    ...
    if (boy.userData._cy != null && boy.userData._cy > PLATEAU_Y + 3 && climbY == null) {
      ...
      if (climbYAt(boy.position.x, oz) != null) { boy.position.z = oz; vel.z = 0 }       // zだけ戻す＝x方向にスライド
      else if (climbYAt(ox, boy.position.z) != null) { boy.position.x = ox; vel.x = 0 }  // xだけ戻す＝z方向にスライド
      else { boy.position.x = ox; boy.position.z = oz; vel.x = 0; vel.z = 0 }                 // 両方ダメ＝戻す
      climbY = climbYAt(boy.position.x, boy.position.z); if (climbY == null) climbY = boy.userData._cy
    }
    ...
    const onSunRoof = area === 'yato' && climbYAt(boy.position.x, boy.position.z) != null // サンライズの屋上にいる
    const nearSunDoor = area === 'yato' && !onSunRoof && Math.hypot(boy.position.x - 3012, boy.position.z - 25) < 6 // 入口の前（坂上=南側・外階段の足元）
    // いちばん近い人を話し相手に
    ...
    lieBtn.style.display = (dialogue || onSunRoof) ? 'none' : 'block' // 屋上では「ねころぶ」を出さない（地面の高さに落ちるため）
...
window.__proto3d = {
  ...
  sunRoof(on) { area = 'yato'; if (on) sunGoRoof(); else sunLeaveRoof() }, // 検証用：サンライズ屋上へ登る/おりる
  sunRoofY: { get t6() { return SUN_ROOF.t6 }, get top() { return SUN_ROOF.top } }, // 検証用：屋上の高さ
