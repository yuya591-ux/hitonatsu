...
function heightAt(x, z) {
  ...
  if (x > 500) {
    ...
    // 長い坂道（尾根）：z方向に南→北で登る(smoothstepで下端/上端なだらか)＋x方向は道から離れると平地へ
    const along = Math.max(0, Math.min(1, (z - SLOPE.z0) / (SLOPE.z1 - SLOPE.z0)))
    const across = Math.exp(-((x - SLOPE.x) * (x - SLOPE.x)) / (2 * SLOPE.w * SLOPE.w))
    const s = SLOPE.h * (along * along * (3 - 2 * along)) * across
...
const townNightLights = [] // 夜に灯る街のあかり（窓・街灯・自販機）。nfで点灯＝夜のエモさ
{
  ...
  makeGameShop(T.x - 70, T.z - 54, -Math.PI / 2) // 坂の途中(中ほど)の道の東脇＝ビスコ（店先は西=道側）
  ...
  makeDagashi(T.x - 68, T.z - 88, -Math.PI / 2) // 坂の一番下＝しんみせ（新店・店先は西=道側）
