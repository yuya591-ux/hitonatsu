...
const SUNRISE_HILL = { x: TOWN.x - 82, z: TOWN.z + 16, h: 11, w: 16, d: 16 } // 丘の頂上はマンションの北。坂道は南北に走り、北(奥)が上り・南(手前)が下り。マンションは坂道の西脇(道中)に建ち入口は東の道へ向く
const MANSION = { x: TOWN.x - 82, z: TOWN.z - 26 } // マンション本体＝南北の坂道の西脇に建つ。入口は東(道)へ向き、奥(西)に立体駐車場。坂を登る(北)と左手にマンション入口
...
const townNightLights = [] // 夜に灯る街のあかり（窓・街灯・自販機）。nfで点灯＝夜のエモさ
{
  ...
  const TGX = T.x - 22, TGZ = T.z + 20 // 地面メッシュの中心（西へ＋南へ拡張＝南北の坂道の“下り(南)”にビスコ/しんみせの余地）
  const tgeo = new THREE.PlaneGeometry(274, 240, 137, 120); tgeo.rotateX(-Math.PI / 2) // 西＋南へ拡張（坂道に沿ったランドマーク配置の余地）
  ...
  makeRoadRibbon(T.x - 74, T.z - 80, T.x - 78, T.z + 26, 8) // 南北の坂道：南(手前)=下り → 北(奥)=上り。マンションは道の西脇(道中)・入口は東の道へ向く
  // 上り(北)＝頂上→北寺尾の町→鶴見駅方面／下り(南)＝ビスコ・しんみせ→二つ池
  makeRoadRibbon(T.x - 74, T.z - 80, T.x - 4, T.z - 48, 6) // 坂下(南)を町の本通りへ接続
  ...
  makeGround(T.x - 88, T.z + 48) // マリノスのグラウンド＝丘の北の平地（森を迂回して小学校へ向かう道中の左手・確定レイアウト）
  ...
  makeGameShop(T.x - 66, T.z - 56, Math.PI / 2) // 坂道を南(下り)へ下った道の東脇＝ビスコ（店先は西=道側）
  ...
  makeDagashi(T.x - 60, T.z - 74, Math.PI / 2) // さらに南へ坂を下った坂下の交差点角＝しんみせ（店先は西=道側）
  ...
  makePondPark(T.x + 5, T.z + 46) // 二つ池＝学校の北寄り（裏山の手前のフラット地。確定レイアウトの“北”に寄せる）
...
function update(dt) {
  ...
  if (mode === 'walk') {
    ...
    if (area === 'field') {
      ...
      boy.position.x = THREE.MathUtils.clamp(boy.position.x, TOWN.x - 152, TOWN.x + 78)
      boy.position.z = THREE.MathUtils.clamp(boy.position.z, TOWN.z - 88, TOWN.z + 96)
    } else { // 神社
