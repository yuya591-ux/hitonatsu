...
// 外階段：街側(SE)の長い辺 pt0→pt1 に沿う一本の直線階段（高さマップで歩けるよう“1点1高さ”＝単調）。地上y0→屋上top
const SUN_STAIR = (() => { const A = SUN_POLY[0], B = SUN_POLY[1], dx = B[0] - A[0], dz = B[1] - A[1], L = Math.hypot(dx, dz), ux = dx / L, uz = dz / L, nx = -uz, nz = ux, off = 3.0, wT = 3.2, sA = 2.5, sB = 38, fx = A[0] + ux * sA + nx * off, fz = A[1] + uz * sA + nz * off; return { ax: A[0], az: A[1], ux, uz, nx, nz, off, wT, hw: wT / 2, sA, sB, footX: fx, footZ: fz, y0: heightAtYato(fx, fz) } })()
function sunriseYatoClimbY(x, z) { // 平らな屋上＋歩ける外階段＋上端の橋。輪郭/帯の外はnull＝縁で落下防止が効く
  const S = SUN_STAIR, dx = x - S.ax, dz = z - S.az, s = dx * S.ux + dz * S.uz, o = dx * S.nx + dz * S.nz
  if (s >= S.sA && s <= S.sB && o >= S.off - S.hw && o <= S.off + S.hw) return S.y0 + (s - S.sA) / (S.sB - S.sA) * (SUN_ROOF.top - S.y0) // 外階段（一段ずつ登れる斜面）
  if (s >= S.sB - 3 && s <= S.sB + 3 && o >= -2 && o <= S.off + S.hw) return SUN_ROOF.top // 階段上端→屋上の橋（平ら）
  if (pointInSunPoly(x, z)) return SUN_ROOF.top // 平らな陸屋上
  return null
}
...
function buildShishigaya() {
  ...
  { const poly = SUNRISE_POLY
    ...
    if (tris && tris.length) for (const t of tris) { const A = poly[t[0]], B = poly[t[1]], C = poly[t[2]]; pushTri(flatTop, [A[0], top, A[1]], [B[0], top, B[1]], [C[0], top, C[1]]) } // 平らな陸屋上（雁行の内側ぜんぶ同じ高さ）
  ...
  // ───── サンライズの屋上：外周パラペット＋街側の外階段（歩いて一段ずつ登れる本物の階段）。高さは SUN_ROOF/SUN_STAIR と一致 ─────
  { const grp = new THREE.Group(); grp.name = 'sunriseRoofAccess'; scene.add(grp)
    const conc = toon(0xbab5a8), rail = toon(0x9a9a90), step = toon(0xc7c2b4)
    ...
    const top = SUN_ROOF.top
    // 外周パラペット（腰高＋笠木）＝落ちない安心感。階段がつながる pt0→pt1 の辺(k=0)だけは開けておく（その辺は階段の手すりが縁になる）
    for (let k = 1; k < SUN_POLY.length; k++) { const a = SUN_POLY[k], b = SUN_POLY[(k + 1) % SUN_POLY.length], len = Math.hypot(b[0] - a[0], b[1] - a[1]); if (len < 1) continue
      const cx = (a[0] + b[0]) / 2, cz = (a[1] + b[1]) / 2, ang = Math.atan2(b[1] - a[1], b[0] - a[0])
      RM(new THREE.BoxGeometry(len, 1.1, 0.24), conc, cx, top + 0.55, cz, ang) // 立ち上がり
      RM(new THREE.BoxGeometry(len, 0.1, 0.34), rail, cx, top + 1.12, cz, ang) } // 笠木（手すり天端）
    // 外階段（直線・歩いて登れる）。SUN_STAIR と同じ帯。斜めの桁(solid)＋踏面＋蹴上げ＋手すり＝仮設でなく“ちゃんとした階段”
    { const S = SUN_STAIR, P = (s, o) => [S.ax + S.ux * s + S.nx * o, S.az + S.uz * s + S.nz * o], head = Math.atan2(S.ux, S.uz)
      const run = S.sB - S.sA, rise = top - S.y0, slope = Math.atan2(rise, run), slopeLen = Math.hypot(run, rise)
      const mid = P((S.sA + S.sB) / 2, S.off), my = (S.y0 + top) / 2
      RM(new THREE.BoxGeometry(S.wT, 0.7, slopeLen), conc, mid[0], my - 0.45, mid[1], head, -slope) // 斜めの桁（階段の底＝中身が詰まって見える）
      const N = 26, dRun = run / N, dY = rise / N
      for (let i = 1; i <= N; i++) { const s = S.sA + i * dRun, p = P(s, S.off), sy = S.y0 + i * dY
        RM(new THREE.BoxGeometry(S.wT, 0.16, dRun + 0.25), step, p[0], sy + 0.02, p[1], head) // 踏面
        const r = P(s - dRun / 2, S.off); RM(new THREE.BoxGeometry(S.wT, dY + 0.05, 0.1), conc, r[0], sy - dY / 2 + 0.02, r[1], head) } // 蹴上げ
      for (const side of [-1, 1]) { // 手すり：支柱＋斜めの笠木
        for (let i = 0; i <= N; i += 3) { const s = S.sA + i * dRun, p = P(s, S.off + side * (S.hw - 0.1)), sy = S.y0 + i * dY; RM(new THREE.CylinderGeometry(0.05, 0.05, 1.0, 5), rail, p[0], sy + 0.5, p[1]) }
        const hp = P((S.sA + S.sB) / 2, S.off + side * (S.hw - 0.1)); RM(new THREE.BoxGeometry(0.09, 0.09, slopeLen), rail, hp[0], my + 1.0, hp[1], head, -slope) }
      const bp = P(S.sB, (S.off - 0.6) / 2); RM(new THREE.BoxGeometry(S.off + S.wT + 1.6, 0.22, S.wT + 1.4), conc, bp[0], top, bp[1], head) // 上端→屋上へ渡る平らな橋
    }
  }
...
boy.position.set(3001, heightAt(3001, 13), 13); boy.rotation.y = Math.atan2(SUN_STAIR.footX - 3001, SUN_STAIR.footZ - 13) // ゲーム開始位置＝獅子ヶ谷サンライズ北寺尾の入口(外階段の足元)・階段の方を向く（ユーザー要望2026-06-22）
outlineObj(boy, 0.03)
...
window.__build = '20260622-walk-stairs-flat-roof' // ビルド識別（HTMLのみ変更時もバンドル名を変えて自動更新を効かせるため）
...
window.__proto3d = {
  ...
  sunRoof(on) { area = 'yato'; if (on) { boy.position.set(3018, SUN_ROOF.top, 4) } else { boy.position.set(SUN_STAIR.footX, SUN_STAIR.y0, SUN_STAIR.footZ) } boy.userData._cy = null }, // 検証用：屋上/階段の足元に置く
  sunRoofY: { get top() { return SUN_ROOF.top }, get y0() { return SUN_STAIR.y0 } }, // 検証用：屋上top/階段足元y0
  sunClimbY(x, z) { return climbYAt(x, z) }, // 検証用：その地点の歩行面の高さ（階段の段々を確認）
