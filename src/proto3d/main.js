...
function heightAtYato(x, z) { // 実標高をバイリニア補間。±SG.half外は縁の値で頭打ち
  ...
  const r = Math.hypot(x - 3008, z - 8), fall = r < 145 ? 1 - THREE.MathUtils.smoothstep(r, 60, 145) : 0
  if (fall > 0) { const upProj = (x - 3008) * 0.5 + (z - 8) * (-0.866); return e + upProj * 0.22 * fall } // 上手SE↑/下手NNW↓
  return e
}
...
function buildShishigaya() {
  ...
  // ───── サンライズ直下の坂のランドマーク（住人情報）：谷側(NW)の駐車場入口(シャッター兼マンション入口)＋坂を下った先のゲームショップ「ディスコ」 ─────
  { const grp = new THREE.Group(); grp.name = 'sunriseSurround'; scene.add(grp)
    ...
    const sx = 2958, sz = 82, sg = heightAtYato(sx, sz), shop = new THREE.Group(); shop.position.set(sx, sg, sz); shop.rotation.y = Math.atan2(3008 - sx, 8 - sz); grp.add(shop) // (2) ゲームショップ「ディスコ」：坂下の少し低い所。正面を上手(サンライズ側)へ
    ...
    const signTex = (() => { const c = document.createElement('canvas'); c.width = 256; c.height = 64; const x = c.getContext('2d'); x.fillStyle = '#b5462f'; x.fillRect(0, 0, 256, 64); x.fillStyle = '#fff8e8'; x.font = 'bold 34px sans-serif'; x.textAlign = 'center'; x.textBaseline = 'middle'; x.fillText('ゲーム  ディスコ', 128, 36); return new THREE.CanvasTexture(c) })()
    shop.add(mk(new THREE.PlaneGeometry(9.4, 2.0), new THREE.MeshBasicMaterial({ map: signTex, side: THREE.DoubleSide }), 0, 4.7, 4.05)) } // 看板「ゲーム ディスコ」
...
window.__build = '20260621-shishigaya-geo18' // ビルド識別（HTMLのみ変更時もバンドル名を変えて自動更新を効かせるため）
