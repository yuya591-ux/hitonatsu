...
// ───────── 新エリア『獅子ヶ谷（本格トレース）』x≈3000・x>2600で分離（実在の谷戸地形を別エリアに新規構築。現エリアには非接触）─────────
// 実在：下末吉台地(NW)の縁に谷戸が樹枝状に食い込み、谷底は水田、背骨を小川が下る。横溝屋敷=谷の口(御園)→二ツ池+田園→西谷広場→市民の森の丘(北向き斜面)。尾根に峠道=車地蔵。
// 向き：+z=北=谷の口=低い(鶴見川側)／-z=南=谷の奥=森・台地=高い。標高は実在6→27mを比例圧縮し谷底1.5m→台地/森頂~19m。
const YATO = {
  x: 3000, z: 0,
  // 谷の背骨（中心線）：各点 [x, z, 谷底標高]（口=北/+z/低 → 奥=南/-z/高）
  spine: [[3000, 132, 1.2], [3000, 95, 2.2], [2992, 50, 3.0], [3004, 5, 5.5], [3016, -35, 8.5]],
  branchA: [[2992, 30, 3.2], [2962, 16, 5.2], [2934, 2, 7.6]],   // 枝谷A＝西の台地へ食い込む指
  branchB: [[3008, -8, 6.4], [3034, -28, 8.8], [3060, -52, 11.5]] // 枝谷B＝東/南の森へ食い込む指
}
...
// 新エリア『獅子ヶ谷』の地形：台地(西)・市民の森の丘(南東)を立ち上げ、そこへ谷戸(背骨+枝谷2本)を樹枝状に彫り、谷底は水田の標高、北端は鶴見川の低地。
function heightAtYato(x, z) {
  const Y = YATO
  let h = 1.5 + 12.5 * smoothstep01((130 - z) / 240) // 大局：北(+z)=低→南(-z)=高 の谷勾配
  const daiti = 19 * smoothstep01((2945 - x) / 30) * smoothstep01((60 - z) / 70) // 台地（西・-x側）≒19m
  if (daiti > h) h = daiti
  const fdx = x - 3035, fdz = z + 88 // 市民の森の丘（南東・-z,+x）peak≒16m
  const mori = 16 * Math.exp(-(fdx * fdx / (2 * 48 * 48) + fdz * fdz / (2 * 52 * 52)))
  if (mori > h) h = mori
  h += ((h > 2) ? 0.35 : 0.1) * Math.sin(x * 0.12) * Math.cos(z * 0.12) // ゆるい起伏
  h = carveValley(h, x, z, Y.spine, 7, 16)    // 主谷（せせらぎ・谷戸田）
  h = carveValley(h, x, z, Y.branchA, 5, 13)  // 枝谷A（西の台地へ）
  h = carveValley(h, x, z, Y.branchB, 5, 13)  // 枝谷B（南の森へ）
  const sdx = (x - 2968) / 16, sdz = (z + 52) / 14 // 峠道の鞍部（森と台地の間の尾根を少し下げる）
  const saddle = smoothstep01(1 - Math.sqrt(sdx * sdx + sdz * sdz))
  if (saddle > 0 && h > 13) h = h * (1 - saddle * 0.6) + 13 * (saddle * 0.6)
  const pdx = x - 2997, pdz = z - 54, pdd = Math.sqrt(pdx * pdx + pdz * pdz) // 二ツ池＝谷中の浅い窪地（水が岸より低く溜まる）
  const pondK = smoothstep01((10.5 - pdd) / 6)
  if (pondK > 0) h = h * (1 - pondK) + 2.7 * pondK
  const ndx = x - 3040, ndz = z + 70, ndd = Math.sqrt(ndx * ndx + ndz * ndz) // 市民の森の新池（森の窪み）
  const npK = smoothstep01((7 - ndd) / 4)
  if (npK > 0 && h > 13.5) h = h * (1 - npK) + 13.5 * npK
  const s2dx = x - 3026, s2dz = z + 26, s2dd = Math.sqrt(s2dx * s2dx + s2dz * s2dz) // 下谷の池（森の窪み）
  const s2K = smoothstep01((6.5 - s2dd) / 4)
  if (s2K > 0 && h > 8.5) h = h * (1 - s2K) + 8.5 * s2K
  const riv = smoothstep01((z - 116) / 12) // 鶴見川の低地（北端 z>116）≒1.2m
  if (riv > 0 && h > 1.2) h = h * (1 - riv) + 1.2 * riv
  return h
}
function heightAt(x, z) {
  if (x > 2600) return heightAtYato(x, z) // 新エリア『獅子ヶ谷（本格トレース）』。神社(x>1500)より先に判定
  if (x > 1500) {
...
{
  const Y = YATO
  const YGX = Y.x, YGZ = Y.z + 5 // 地面メッシュの中心（z[-140,150]を覆う）
  const ygeo = new THREE.PlaneGeometry(300, 290, 200, 193); ygeo.rotateX(-Math.PI / 2) // ~1.5m格子＝細い谷戸の谷底もなめらかに
  const ypos = ygeo.attributes.position, ycol = []
  const cLow = new THREE.Color(0xb6ad99), cGrass = new THREE.Color(0x86b257), cDark = new THREE.Color(0x6f9a47) // 街の地面と同じ色系で統一
  for (let i = 0; i < ypos.count; i++) {
    ...
    c.lerp(cDark, THREE.MathUtils.smoothstep(y, 12, 20))
  ...
  yg.position.set(YGX, 0, YGZ); yg.receiveShadow = true; scene.add(yg)
  ...
  makeRicePaddy(3003, 88, 10, 12) // 横溝屋敷前（御園）の谷戸田
  makeRicePaddy(2998, 72, 9, 11)  // 二ツ池の北の田
  makeRicePaddy(3003, 34, 9, 11)  // 二ツ池の南の田
  makeRicePaddy(3008, 16, 8, 10)  // 西谷寄りの田
  makeRicePaddy(2951, 14, 8, 11)  // 枝谷Aの棚田（西の指）
  makeStream([[3010, 6], [3006, 30], [3009, 52], [3002, 76], [3003, 98], [3002, 120]], 1.7) // せせらぎ（小川アメニティ）＝谷底の背骨を北へ下る（二ツ池の東を通す）
  makeRoadRibbon(3006, 116, 3008, 92, 1.3, false) // あぜ道：川口→田を巡り西谷へ（土の小径）
  makeRoadRibbon(3008, 92, 3006, 70, 1.3, false)
  makeRoadRibbon(3006, 70, 3008, 48, 1.3, false)
  makeRoadRibbon(3008, 48, 3010, 20, 1.3, false)
  makeRoadRibbon(2960, 26, 2950, 10, 1.2, false) // 枝谷Aへ入る畦
  let tp = 0, tt = 0 // 雑木林：森の丘・台地の斜面に（谷底の田には生やさない＝高さ9m以上へ）
  while (tp < 72 && tt < 1100) {
    tt++
    const x = 2902 + Math.random() * 196, z = -118 + Math.random() * 176
    if (heightAt(x, z) < 9) continue
    makeTree(x, z, 0.75 + Math.random() * 0.7); tp++
  }
}
...
}
{
  ...
  // ───────── 新エリア『獅子ヶ谷（本格トレース）』のランドマーク（横溝屋敷・二ツ池・車地蔵＋峠道）。x>2600で既存に非接触 ─────────
...
window.__build = '20260620-shishigaya-yato' // ビルド識別（HTMLのみ変更時もバンドル名を変えて自動更新を効かせるため）
...
function update(dt) {
  ...
  if (mode === 'walk') {
    ...
    if (area === 'field') {
      ...
      boy.position.x = THREE.MathUtils.clamp(boy.position.x, YATO.x - 145, YATO.x + 145)
      boy.position.z = THREE.MathUtils.clamp(boy.position.z, YATO.z - 135, YATO.z + 145)
    } else { // 神社
...
function warpBoyTo(sx, sy) {
  ...
  if (area === 'field') {
    ...
    wx = THREE.MathUtils.clamp(wx, YATO.x - 145, YATO.x + 145); wz = THREE.MathUtils.clamp(wz, YATO.z - 135, YATO.z + 145)
  } else { // 神社
