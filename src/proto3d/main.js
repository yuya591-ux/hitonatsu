...
const MOUNT2 = { x: TOWN.x - 28, z: TOWN.z + 64, h: 32, w: 36, d: 26 } // 裏山の南西に足す“もう一つの峰”(972,64)＝新道の上り先を裏山と同じ高さに盛り上げ一体化（ユーザー要望）。南の町は守る(zクリップ)
...
const bonOdori = new THREE.Group(); bonOdori.visible = false; scene.add(bonOdori) // 盆踊り会場（櫓＋提灯）＝小学校の校庭。開催日だけ姿を見せる
{
  ...
  // 裏山の雑木（斜面に点々と＝木立の山）
  for (const [tx, tz, ts] of [[T.x - 22, T.z + 50, 1.1], [T.x + 26, T.z + 56, 1.2], [T.x - 6, T.z + 66, 1.0], [T.x + 16, T.z + 72, 1.1], [T.x - 50, T.z + 78, 0.95], [T.x + 36, T.z + 46, 1.0], [T.x - 14, T.z + 80, 0.9]]) makeTree(tx, tz, ts) // ※(970,66)は新しい峰の上＝南西(950,78)へ退避
  ...
  makeRoadRibbon(T.x - 50, T.z + 64, T.x - 28, T.z + 63, 4, false, true)   // グイッと上って“新しい峰(MOUNT2)”の上へ
  makeRoadRibbon(T.x - 28, T.z + 63, T.x + 8, T.z + 55, 4, false, true)    // 新しい峰→峠道(既存の裏山)へ＝高台どうしが一体に
