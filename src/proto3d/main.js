...
function heightAt(x, z) {
  ...
  if (x > 500) {
    ...
    // ── マリノスのグラウンド＝“崖下の平らな運動場”(≒4m)。東端(ビスコの少し手前)を45度の急バンクで立ち上げてビスコ(8.8)へ＝段差をしっかり見せる（ユーザー要望2026-06-18・B案）──
    {
      const gk = smoothstep01((x - 842) / 12) * smoothstep01((905 - x) / 4.5) * smoothstep01((z + 14) / 10) * smoothstep01((30 - z) / 10) // 東端 4.5m幅で4→8＝45度バンク
    ...
    // ── 迂回路の谷を埋める：マンション台地のすぐ北(z-36〜-12)にできた窪みを“西へ下る坂”の高さでなめらかに繋ぐ＝道がいきなり下って上るのを解消（ユーザー指摘2026-06-18）──
    {
      ...
      const fk = smoothstep01((x - 856) / 10) * smoothstep01((901 - x) / 8) * smoothstep01((-12 - z) / 8) * smoothstep01((z + 36) / 10)
