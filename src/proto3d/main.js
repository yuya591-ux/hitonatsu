...
const SLOPE = { x: TOWN.x - 76, z0: TOWN.z - 96, z1: TOWN.z + 4, h: 22, w: 30 }
const SUNRISE_HILL = SLOPE // 互換（旧名参照が残っていても動くように）
const MANSION = { x: TOWN.x - 94, z: TOWN.z - 26 } // マンション本体＝坂道(尾根の道)の西脇・道より一段下がった敷地。入口は東(道)へ向く
...
// 長い坂道の高さ(南→北)。一番下(しんみせ)→登る→途中に平らな踊り場(ビスコ)→また登る→上(マンション〜頂上)。
function slopeHeight(z) {
  const zb = TOWN.z - 88, zl0 = TOWN.z - 60, zl1 = TOWN.z - 48, ztop = TOWN.z - 4, hL = 7, hT = 22
  if (z <= zb) return 0
  if (z < zl0) return hL * smoothstep01((z - zb) / (zl0 - zb)) // 下→踊り場へ登る
  if (z < zl1) return hL // 踊り場（平ら＝ビスコの前で道がまっすぐ）
  if (z < ztop) return hL + (hT - hL) * smoothstep01((z - zl1) / (ztop - zl1)) // 踊り場→上へ登る（途中にマンション）
  return hT // 頂上の平地（北寺尾）
}
function heightAt(x, z) {
  ...
  if (x > 500) {
    ...
    // 長い坂道（尾根）：高さは slopeHeight(z)（途中に踊り場）＋x方向は道から離れると平地へ
    const across = Math.exp(-((x - SLOPE.x) * (x - SLOPE.x)) / (2 * SLOPE.w * SLOPE.w))
...
const townNightLights = [] // 夜に灯る街のあかり（窓・街灯・自販機）。nfで点灯＝夜のエモさ
{
  ...
  // ── 地形に沿う道路リボン（坂でも浮かない帯）。p0→p1 を結ぶ。センターライン付き。──
  function makeRoadRibbon(x0, z0, x1, z1, width, centerline = true) {
    const len = Math.hypot(x1 - x0, z1 - z0), segs = Math.max(8, Math.round(len / 3))
    const dx = (x1 - x0) / len, dz = (z1 - z0) / len, px = -dz, pz = dx // 進行方向と垂直
    const mk = (w, yoff, col, unlit) => {
      const verts = [], uvs = [], idx = []
      ...
      const mat = new THREE.MeshToonMaterial({ color: col, gradientMap: GRAD, map: unlit ? null : watercolorTex })
      const m = new THREE.Mesh(geo, mat); m.receiveShadow = true; scene.add(m); return m
    }
    mk(width, 0.04, 0x8f8d88, false)
    if (centerline) mk(0.3, 0.06, 0xcfc9bb, false)
  }
  // マンション正面(南)に平行な“坂道”＝東(左手)上り・西(右手)下り。マンションはこの道の道中に建つ。
  makeRoadRibbon(T.x - 76, T.z - 92, T.x - 76, T.z + 12, 9) // 長く大きい坂道（尾根の道）：南(手前)=下り → 北(奥)=上り。マンションは坂の約7割地点の西脇
  // 上り(北)＝頂上→北寺尾の町→鶴見駅方面／下り(南)＝ビスコ(坂の途中)→しんみせ(一番下)→二つ池
  makeRoadRibbon(T.x - 76, T.z - 86, T.x - 4, T.z - 52, 6) // 坂下(南)を町の本通りへ接続
  makeRoadRibbon(T.x - 80, T.z - 26, T.x - 91, T.z - 26, 5, false) // マンションへ左折で入る短い下り坂(私道)：尾根の道→西へ一段下って入口へ
  ...
  // ── 森（森山＝立花学園グラウンドの名残）＝マンション背面(西)と小学校の間。森があるので小学校へ直進できず迂回する ──
  for (const [tx, tz, ts] of [[910, -12, 1.4], [908, -26, 1.5], [912, -2, 1.3], [905, -40, 1.4], [909, -19, 1.3], [890, -26, 1.5], [884, -34, 1.4], [896, -32, 1.3], [900, -22, 1.4], [878, -28, 1.3]]) makeTree(tx, tz, ts)
  ...
  makeGround(T.x - 116, T.z + 4) // マリノスのグラウンド＝マンション背面(西)の道中・森を迂回して小学校へ向かう途中の左手
  ...
  function makeMansion(cx, cz) {
    const g = new THREE.Group()
    const floors = 7, units = 6, FH = 2.6, baseH = 3.2, W = units * 2.6, D = 8.5 // 横に太く（立体駐車場の余地）
    ...
    mergedOutline(g, 0.05); addContactShadow(g, Math.max(W, D) * 0.7); addCollider(cx - 2.6, cz, 3.8); scene.add(g) // コライダーは建物の西側だけ＝東の入口/私道/坂道は歩ける
    for (const gl of winGlows) townNightLights.push({ m: gl, base: 0.85, ph: Math.random() * 6 })
  }
  ...
  makeGameShop(T.x - 86, T.z - 54, Math.PI / 2) // 坂の途中の“踊り場”の左手(西)＝ビスコ（店先は東=道側／登ると左手に見える）
  ...
  makeDagashi(T.x - 84, T.z - 86, Math.PI / 2) // 坂の一番下＝しんみせ（新店・道の西脇・店先は東=道側）
  ...
  function makeHighSchool(cx, cz, rot) {
    ...
    g.traverse((o) => { if (o.isMesh) o.castShadow = true })
    g.position.set(cx, heightAt(cx, cz), cz); g.rotation.y = rot
    mergedOutline(g, 0.05); addContactShadow(g, Math.max(W, D) * 0.6); addCollider(cx, cz, Math.max(W, D) * 0.42); scene.add(g)
    // 土の校庭（南側）＋バックネットの気配
  ...
  function makeKindergarten(cx, cz, rot) {
    ...
    g.traverse((o) => { if (o.isMesh) o.castShadow = true })
    g.position.set(cx, heightAt(cx, cz), cz); g.rotation.y = rot
    mergedOutline(g, 0.04); addContactShadow(g, 5); addCollider(cx, cz, 3.4); scene.add(g)
  }
  ...
  // グラウンドを突っ切ってしんみせ裏へ抜ける踏み跡の近道（マリノスG→しんみせ。確定レイアウトの裏道）
  { const path = new THREE.Mesh(new THREE.PlaneGeometry(3, 58), new THREE.MeshToonMaterial({ color: 0xc2a878, gradientMap: GRAD, map: watercolorTex })); path.rotation.x = -Math.PI / 2; path.rotation.z = 0.04; path.position.set(T.x - 86, heightAt(T.x - 86, T.z - 24) + 0.05, T.z - 24); scene.add(path) }
