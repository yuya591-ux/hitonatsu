...
function heightAtYato(x, z) { // 実標高をバイリニア補間。±SG.half外は縁の値で頭打ち
  ...
  // サンライズ北寺尾は実地では結構な急勾配の坂の途中（DEM5Aが均すため局所補正）。中心(3008,8)を軸に南(高)→北=二ツ池側(低)へ傾ける
  const r = Math.hypot(x - 3008, z - 8), fall = r < 115 ? 1 - THREE.MathUtils.smoothstep(r, 55, 115) : 0
  return e - (z - 8) * 0.12 * fall
}
...
window.__build = '20260621-shishigaya-geo14' // ビルド識別（HTMLのみ変更時もバンドル名を変えて自動更新を効かせるため）
