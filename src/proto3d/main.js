...
const SUNRISE_HILL = { x: TOWN.x - 104, z: TOWN.z - 22, h: 11, w: 13, d: 16 } // 丘の頂上はマンションの西。坂道はマンション正面に平行な東西の道＝西(左手)が上り・東(右手)が下り
const MANSION = { x: TOWN.x - 82, z: TOWN.z - 26 } // マンション本体＝坂道の道中(丘の東flank)に建つ。正面(南)に平行な坂道、左(西)が上り・右(東)が下り
...
const townNightLights = [] // 夜に灯る街のあかり（窓・街灯・自販機）。nfで点灯＝夜のエモさ
{
  ...
  const TGX = T.x - 22, TGZ = T.z + 35 // 地面メッシュの中心（西へさらに拡張＝マンション正面右の下り坂・ビスコ/しんみせの余地）
  const tgeo = new THREE.PlaneGeometry(274, 210, 137, 105); tgeo.rotateX(-Math.PI / 2) // 西へ拡張（坂道に沿ったランドマーク配置の余地）
  ...
  makeRoadRibbon(T.x - 150, T.z - 42, T.x + 4, T.z - 50, 8) // 西(左手=上り)→東(右手=下り)。マンション正面に平行な坂道
  // 西の上り＝頂上→北寺尾の町→鶴見駅方面／東の下り＝ビスコ・しんみせ→二つ池
  makeRoadRibbon(T.x + 4, T.z - 50, T.x, T.z - 28, 6) // 東端(坂下)を町の本通りへ接続
  ...
  makeGameShop(T.x - 48, T.z - 46, 0) // 坂道を東(右手)へ下った道沿い＝ビスコ（坂の上寄り・公園と高校の間）
  ...
  makeDagashi(T.x - 15, T.z - 56, 0) // さらに東へ坂を下った坂下の交差点角＝しんみせ（確定レイアウト）
