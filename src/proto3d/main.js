...
function update(dt) {
  ...
  if (audioStarted) {
    ...
    // M2：蝉の息継ぎ＝たまに6秒かけて蝉がすっと静まり、また戻る（谷=0.42）。風だけになる“間”の瞬間
    cicBreathCd -= dt; if (cicBreathCd <= 0 && cicBreathT <= 0) { cicBreathCd = 130 + Math.random() * 120; cicBreathT = 6 }
    if (cicBreathT > 0) cicBreathT -= dt
    const cicBreath = cicBreathT > 0 ? (0.42 + 0.58 * Math.abs((cicBreathT - 3) / 3)) : 1 // 6→3秒で下がり3→0で戻る（中央が谷）
    const cicadaSwell = (0.68 + 0.32 * (0.5 + 0.5 * Math.sin(tsec * 0.12)) + 0.06 * Math.sin(tsec * 0.5 + 1.0)) * (1 - weather * 0.75) * cicBreath // 夕立で蝉が静かに＋M2の息継ぎ
